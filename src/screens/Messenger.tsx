...
const Messenger = ({
  ...
  useEffect(() => {
    const loadConversations = async () => {
      try {
        const response = await apiFetch(`${API_BASE_URL}/messenger-conversations?role=student&userId=${encodeURIComponent(currentUser)}`);
  ...
  const handleSend = async () => {
    ...
    if (currentPendingFile) {
      try {
        ...
        const response = await apiFetch(`${API_BASE_URL}/messenger-upload-file`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
