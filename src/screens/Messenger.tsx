...
const DEFAULT_AVATAR = require('../../assets/images/BSITLOGO.webp'); // Placeholder avatar for conversations without a custom image
...
// ---------------------------------------------------------------------------
// Stable helpers defined OUTSIDE the component so they never get recreated
// ---------------------------------------------------------------------------
...
const Messenger = ({
  ...
  // ---------------------------------------------------------------------------
  // normalizeMessages – stable via useCallback (only recreated when user changes)
  // ---------------------------------------------------------------------------
  ...
  const markConversationAsRead = useCallback(
    ...
    [currentUser, onUnreadCountChanged]
  );
    ...
    const loadConversations = async () => {
      try {
        ...
        setConversations((prev) => {
          ...
          // Sort by most recent activity so rooms with new messages or that were
          // just created float to the top instead of sticking wherever the API
          // happened to return them / getting appended to the end.
          const sortedMerged = sortByRecency(merged);

          // Avoid triggering a state update if nothing really changed
          if (
        ...
        // ── Seed messages only for conversations that don't have entries yet ──
        ...
        // ── Stabilise the `selected` reference if server data changed ──
        setSelected((prev) => {
          if (!prev) return prev;
          const updated = incoming.find((c) => c.id === prev.id);
          if (!updated) return prev; // server doesn't know this conversation (local room)
          if (!conversationChanged(prev, updated)) return prev; // nothing changed
          // Carry forward local-only fields (isCreatedRoom, members from local create)
    ...
    const loadMessages = async () => {
      try {
        ...
        setMessagesByConversation((prev) => {
          ...
          if (messagesEqual(existing, allNormalized))  return prev; // nothing changed
  ...
  // Keep a stable ref to the latest messages so the 30s timer can read them
  // without messagesByConversation being in its dependency array (which would
  // restart the interval on every message poll).
  ...
  // ---------------------------------------------------------------------------
  // Relative-time ticker (update "Xm ago" labels once per minute)
  // ---------------------------------------------------------------------------
  useEffect(() => {
    const timer = setInterval(() => {
      setConversations((prev) =>
        prev.map((c) => {
          ...
          if (c.time === newTime) return c; // same reference
  ...
  useEffect(() => {
    if (selected) markConversationAsRead(selected.id);
  ...
  const handleSelectConversation = useCallback(
    ...
    [onConversationActiveChange, markConversationAsRead]
  );
...
const styles = StyleSheet.create({
  mobileScreen: { flex: 1, backgroundColor: '#F4F7FB' },
  ...
  screenBackText: { fontFamily: FONT_BODY, fontSize: 14, fontWeight: WEIGHT_EMPHASIS, color: '#111', marginLeft: 2 },
  ...
  convName: { fontFamily: FONT_BODY, fontWeight: WEIGHT_EMPHASIS, color: '#111', flex: 1, minWidth: 0, marginRight: 8 },
  unreadBadge: {
    minWidth: 18,
    height: 18,
    borderRadius: 9,
  ...
  unreadBadgeText: { fontFamily: FONT_BODY, color: '#fff', fontSize: 10, fontWeight: WEIGHT_EMPHASIS },
  ...
  infoActionCardText: { fontFamily: FONT_BODY, fontSize: 14, fontWeight: WEIGHT_EMPHASIS, color: '#222' },
  messageRow: {},
  messageRowThem: { alignSelf: 'flex-start' },
  messageRowMe: { alignSelf: 'flex-end', alignItems: 'flex-end' },
  senderName: { fontFamily: FONT_BODY, color: '#666', marginBottom: 4, fontWeight: WEIGHT_EMPHASIS },
  ...
  timeTooltipText: { fontFamily: FONT_BODY, color: '#fff', fontSize: 11, fontWeight: WEIGHT_EMPHASIS },
  inputArea: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#fff',
  ...
  sendBtnText: { fontFamily: FONT_BODY, color: '#fff', fontWeight: WEIGHT_EMPHASIS },
  emptyPane: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'center',
  ...
  emptyPaneSubtitle: { fontFamily: FONT_BODY, 
    marginTop: 6,
    fontSize: 14,
    color: '#777',
    textAlign: 'center',
  ...
  professionalModalSubtitle: { fontFamily: FONT_BODY, 
    marginTop: 4,
    fontSize: 12,
    lineHeight: 18,
    color: '#6b7280',
  ...
  fileNameText: { fontFamily: FONT_BODY, fontWeight: WEIGHT_EMPHASIS },
