...
export interface AssignmentItem {
  ...
  createdAt?: string;
  // ✅ Mirrors the teacher-side "Disable repository after due" setting
  // (TeacherCourseDetail2.tsx / server.js: repositoryDisabledAfterDue).
  // When true AND the due date has passed, students can no longer submit.
...
// ✅ NEW: "Disable repository after due" (repositoryDisabledAfterDue) closes
// submissions entirely once the due date passes — this is stricter than the
// default behavior, where a late submission is still accepted and simply
// flagged as "late". Being past due alone is NOT enough to lock submission;
// the teacher must have explicitly enabled this setting on the assignment.
...
type DisplayStatus = AssignmentItem['status'] | 'missing';
...
interface AssignmentsProps {
  ...
  // ✅ NEW: lets the student jump to the attempt-selection screen for a
  // game-based assignment they've already played, to choose (or change)
  // which completed attempt counts as their final score.
...
const Assignments = ({
  ...
  // Real rendered width of the assignments grid (onLayout) — same approach as
  // CourseDetail's Assignments tab so cards fill the available space.
  ...
  // 👇 NEW: "Turn in assignment?" confirmation modal (Google Classroom style),
  ...
  // ✅ NEW: which attempt (if any) the student has already picked as their
  // official final score for each game-based assignment, keyed by
  // assignmentId. Used to label the "Select Final Score" action.
  const [selectedGameAttemptIds, setSelectedGameAttemptIds] = useState<Record<string, string | null>>({});
  // Whether that final score was chosen automatically (all attempts used,
  // highest score fell back) rather than picked by the student.
  ...
  const statusFilteredAssignments = useMemo(() => {
    return filter === 'all'
      ? allAssignments
      : allAssignments.filter((a) => getDisplayStatus(a) === filter);
  ...
  // 🌟 UPDATED: was previously counting `classSubmissions` docs, but a
  // student only ever has ONE submission doc per assignment (it gets
  // upserted), so that count could only ever read 0 or 1 — it never
  // reflected how many times the game had actually been played. Now reads
  // from the real per-attempt record instead, which also tells us whether
  // a final score has already been chosen for this assignment.
  ...
  const syncSelectedAssignmentStatus = (status: AssignmentItem['status']) => {
    ...
    // ✅ FIX: Only ever submit the student's OWN items. `assignmentFiles`
    // should already contain just student-added files/links, but if a
    // teacher-attached file ever ends up mixed into this array (e.g. across
    // an unsubmit -> edit -> resubmit cycle), we must not let it slip
    // through here — otherwise it gets duplicated and re-uploaded as if it
    // were a new student submission item. This mirrors the same
  ...
  // ── Assignments grid layout (mirrors CourseDetail's Assignments tab) ──
  const ASSIGNMENT_GAP = 14;
  const ASSIGNMENT_MIN_CARD_WIDTH = 320;
  const assignmentAvailableWidth =
    assignmentGridWidth > 0 ? assignmentGridWidth : Math.min(width * 0.8, 1600);
  ...
  return (
    ...
    >
      ...
      {searchedAndFilteredAssignments.length > 0 ? (
        ...
        >
          {searchedAndFilteredAssignments.map((item) => (
                    ...
                    <View style={styles.gcHeader}>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.gcSubtitle} numberOfLines={2}>
                          {selectedAssignment.instructor} • {selectedAssignment.courseName}
                        </Text>
                  ...
                  <View style={[styles.gcSideCol, isLargeScreen && styles.gcSideColLarge]}>
                    ...
                    <View style={styles.section}>
                      ...
                      {(() => {
                        ...
                        // ✅ NEW: Past due + "Disable repository after due" enabled, and the
                        // student never submitted in time — submission is fully closed.
                        if (locked) {
                          return (
                            <View style={styles.uploadActionsRow}>
                              <View style={styles.lockedSubmissionBox}>
...
const styles = StyleSheet.create({
  ...
  filterDropdownButtonText: { fontFamily: FONT_BODY,
    fontSize: 14,
    color: '#111',
  ...
  courseName: { fontFamily: FONT_BODY, color: '#666', fontSize: 12, fontWeight: WEIGHT_EMPHASIS, marginTop: 6 },
  ...
  recommendationText: { fontFamily: FONT_BODY, fontWeight: WEIGHT_EMPHASIS, fontSize: 12 },
  // ═══════════════════════════════════════════════════════════════════
  ...
  emptyText: { fontFamily: FONT_BODY, fontSize: 14, color: '#999', textAlign: 'center', marginVertical: 20 },
  commentItem: { backgroundColor: '#F9F9F9', borderRadius: 14, padding: 10, marginBottom: 8, borderLeftWidth: 3, borderLeftColor: '#2196F3' },
  ...
  editRow: { marginTop: 4 },
  editInput: { fontFamily: FONT_BODY, borderWidth: 1, borderColor: '#ddd', borderRadius: 14, paddingHorizontal: 12, paddingVertical: 8, color: '#111', backgroundColor: '#fff', fontSize: 14, lineHeight: 20 },
  ...
  // 👇 NEW: green "Turn in assignment?" variant of the delete-confirm modal —
  ...
  previewLoadingText: { fontFamily: FONT_BODY, marginTop: 12, color: '#666', fontSize: 14 },
