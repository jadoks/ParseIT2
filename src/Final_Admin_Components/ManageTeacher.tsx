...
function mapTeacher(item: BackendTeacherItem): TableTeacherItem {
  return {
...
type SortColumn = "teacherId" | "name" | "birthday" | "email" | null;
...
export default function ManageTeacher({ width }: ManageTeacherProps) {
  ...
  const loadTeachersSilently = useCallback(async () => {
    try {
      ...
      console.log("SILENT REFRESH TEACHERS ERROR =>", error);
    }
  }, []);
  ...
  // adds/edits/deletes a teacher elsewhere. Paused whenever the add/edit
  ...
  const sortedTeachers = useMemo(() => {
    ...
    const getValue = (item: TableTeacherItem) => {
      switch (sortColumn) {
        case "teacherId":
          return item.teacherId.toLowerCase();
        case "name":
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchText, sortColumn, sortDirection, teachers.length]);
  ...
  const openRowMenu = (item: TableTeacherItem, key: string) => {
  ...
  const handleSubmitTeacher = async (payload: TeacherFormPayload) => {
    ...
    if (isCreating) {
      ...
      setTeachers((prev) => [mapTeacher(optimisticRaw), ...prev]);
    ...
    <View style={styles.screen}>
      ...
      <View style={[styles.toolbar, isMobile && styles.toolbarStack]}>
        ...
        >
          ...
          <Text style={styles.primaryActionButtonText}>Add Teacher</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.tableCardInner}>
          <View style={styles.tableMetaRow}>
            <Text style={styles.tableMetaText}>
              {sortedTeachers.length} {sortedTeachers.length === 1 ? "teacher" : "teachers"}
            </Text>
          </View>
