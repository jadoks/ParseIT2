...
export default function StudentApp({ onLogout, currentStudent }: Props) {
  ...
  const loadCurrentStudentProfile = async () => {
    ...
    try {
      ...
      setRemoteStudentProfile({ firstName: profileData.firstName || currentStudent.firstName, lastName: profileData.lastName || currentStudent.lastName, email: profileData.email || currentStudent.email, profileImage: profileData.profileImage || null, bannerImage: profileData.bannerImage || null, profileImageStoragePath: profileData.profileImageStoragePath || null, bannerImageStoragePath: profileData.bannerImageStoragePath || null });
  ...
  useEffect(() => {
    ...
    const setupNavigation = async () => {
      try {
        if (isFullscreenScreen) { await NavigationBar.setBehaviorAsync('overlay-swipe'); await NavigationBar.setVisibilityAsync('hidden'); }
        else await NavigationBar.setVisibilityAsync('visible');
  ...
  const getBase64FromUri = async (uri: string) => {
    if (Platform.OS === 'web') {
      ...
      return await new Promise<string>((resolve, reject) => {
        ...
        reader.onloadend = () => { const result = reader.result; if (typeof result !== 'string') { reject(new Error('Failed to read file as base64.')); return; } resolve(result.includes(',') ? result.split(',')[1] : result); };
  ...
  const handleChangeProfileImage = async (image: any) => {
    ...
    try {
      setCurrentUserAvatar(image); setHasImageChanged(true);
      if (!image?.uri) return;
      ...
      setRemoteStudentProfile((prev) => ({ ...(prev || {}), firstName: prev?.firstName || currentStudent.firstName, lastName: prev?.lastName || currentStudent.lastName, email: prev?.email || currentStudent.email, profileImage: savedData.profileImage, bannerImage: prev?.bannerImage || currentUserBanner?.uri || currentStudent.bannerImage || null }));
    } catch (error: any) { setCurrentUserAvatar(previousAvatar); console.log('SAVE PROFILE IMAGE ERROR =>', error); Alert.alert('Save Failed', error?.message || 'Unable to save profile image.'); }
  };

  const handleChangeBannerImage = async (image: any) => {
    const previousBanner = currentUserBanner;
    try {
      setCurrentUserBanner(image); setHasImageChanged(true);
      if (!image?.uri) return;
      ...
      setRemoteStudentProfile((prev) => ({ ...(prev || {}), firstName: prev?.firstName || currentStudent.firstName, lastName: prev?.lastName || currentStudent.lastName, email: prev?.email || currentStudent.email, profileImage: prev?.profileImage || currentUserAvatar?.uri || currentStudent.profileImage || null, bannerImage: savedData.bannerImage }));
    } catch (error: any) { setCurrentUserBanner(previousBanner); console.log('SAVE BANNER IMAGE ERROR =>', error); Alert.alert('Save Failed', error?.message || 'Unable to save banner image.'); }
  };

  const handleDrawerEmailUpdated = (nextEmail: string) => {
    setRemoteStudentProfile((prev) => ({ ...(prev || {}), firstName: prev?.firstName || currentStudent.firstName, lastName: prev?.lastName || currentStudent.lastName, email: nextEmail, profileImage: prev?.profileImage || currentUserAvatar?.uri || currentStudent.profileImage || null, bannerImage: prev?.bannerImage || currentUserBanner?.uri || currentStudent.bannerImage || null }));
  };
  ...
  const loadStudentAnnouncements = async (courses: CourseDetailData[]) => {
    try {
      ...
      const groupedAnnouncements = await Promise.all(classIds.map(async (classId) => { const response = await apiFetch(`${API_BASE_URL}/class-announcements/${classId}`); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to load announcements.'); return Array.isArray(data) ? data : []; }));
      ...
      active.forEach((item: any) => { const key = `${item.title}-${item.message}-${item.expiresAt}-${item.bannerKey}`; if (!uniqueMap.has(key)) uniqueMap.set(key, item); });
      const mappedAnnouncements: StudentClassAnnouncement[] = Array.from(uniqueMap.values()).map((item: any) => ({ id: item.id, classIds: Array.isArray(item.classIds) ? item.classIds : [], title: item.title || '', message: item.message || '', bannerKey: typeof item.bannerKey === 'number' ? item.bannerKey : 4, bannerImage: ANNOUNCEMENT_BANNERS[typeof item.bannerKey === 'number' ? item.bannerKey : 4], expiresAt: item.expiresAt || null, createdAt: item.createdAt || null, updatedAt: item.updatedAt || null })).sort((a, b) => toMillis(b.createdAt) - toMillis(a.createdAt));
      setStudentAnnouncements(mappedAnnouncements);
    } catch (error) { console.log('LOAD STUDENT ANNOUNCEMENTS ERROR =>', error); setStudentAnnouncements([]); }
  };
  ...
  const loadJoinedClasses = async () => {
    ...
    } catch (error) { console.log('LOAD JOINED CLASSES ERROR =>', error); setJoinedCourses([]); setStudentAnnouncements([]); }
    finally { setIsLoadingJoinedCourses(false); }
  };
  ...
  const loadStudentSubmissionState = useCallback(async () => {
    ...
    try {
      ...
      normalizeSubmissionList(data).forEach((submission) => {
        ...
        if (status === 'graded') {
          ...
          scoresByAssignment[assignmentId] = { ...(Number.isFinite(numericScore) ? { points: numericScore } : {}), ...(Number.isFinite(numericMaxScore) && numericMaxScore > 0 ? { maxPoints: numericMaxScore } : {}), feedback: submission?.feedback ?? null, gameScore: submission?.gameScore, gameTotalQuestions: submission?.gameTotalQuestions, attemptNumber: submission?.attemptNumber };
        }
    ...
    } catch (error) { console.log('LOAD STUDENT SUBMISSIONS FALLBACK TO LOCAL CACHE =>', error); }
    finally { setHasLoadedAssignmentState(true); }
  }, [currentStudent?.studentId]);

  useEffect(() => { setHasLoadedAssignmentState(false); void loadStudentSubmissionState(); }, [loadStudentSubmissionState]);
  useEffect(() => { if (!currentStudent?.studentId || !hasLoadedAssignmentState) return; void writeStoredAssignmentState(currentStudent.studentId, { files: sharedAssignmentFiles, statuses: sharedAssignmentStatuses, scores: sharedAssignmentScores }); }, [currentStudent?.studentId, hasLoadedAssignmentState, sharedAssignmentFiles, sharedAssignmentScores, sharedAssignmentStatuses]);
  ...
  const handleAddAssignmentComment = (assignmentId: string, content: string) => { if (!content.trim()) return; setSharedAssignmentComments((prev) => ({ ...prev, [assignmentId]: [...(prev[assignmentId] || []), { id: `c${Date.now()}`, author: currentUserName, content, timestamp: new Date().toLocaleString(), isInstructor: false }] })); };
  const handleAddAssignmentFile = (assignmentId: string, file: AssignmentFileUpload) => { setSharedAssignmentFiles((prev) => ({ ...prev, [assignmentId]: [...(prev[assignmentId] || []), file] })); };
  const handleRemoveAssignmentFile = (assignmentId: string, fileId: string) => { setSharedAssignmentFiles((prev) => ({ ...prev, [assignmentId]: (prev[assignmentId] || []).filter((file) => file.id !== fileId) })); };
  const handleUpdateAssignmentStatus = (assignmentId: string, status: AssignmentItem['status']) => {
    setSharedAssignmentStatuses((prev) => { const next = { ...prev, [assignmentId]: status }; void writeStoredAssignmentState(currentStudent.studentId, { files: sharedAssignmentFiles, statuses: next, scores: sharedAssignmentScores }); return next; });
  ...
  const loadCommunityPosts = async () => { try { const response = await apiFetch(`${API_BASE_URL}/community-posts`); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to load community posts.'); setCommunityPosts(Array.isArray(data?.data) ? data.data : []); } catch (error) { console.log('LOAD COMMUNITY POSTS ERROR =>', error); } };
  const handleCreateCommunityPost = async (query: string) => { const trimmedQuery = query.trim(); if (!trimmedQuery) return; try { const response = await apiFetch(`${API_BASE_URL}/community-posts`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: trimmedQuery, authorId: currentStudent.studentId, authorUid: currentStudent.authUid || null, authorRole: 'student', userName: currentUserName, userEmail: currentUserEmail, avatar: normalizeCommunityAvatar(currentUserAvatar) }) }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to create post.'); await loadCurrentStudentProfile(); await loadCommunityPosts(); } catch (error: any) { Alert.alert('Post Failed', error?.message || 'Unable to create post.'); } };
  const handleAddCommunityAnswer = async (postId: string, message: string) => { const trimmedMessage = message.trim(); if (!trimmedMessage) return; try { const response = await apiFetch(`${API_BASE_URL}/community-posts/${postId}/answers`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: trimmedMessage, authorId: currentStudent.studentId, authorUid: currentStudent.authUid || null, authorRole: 'student', userName: currentUserName, avatar: normalizeCommunityAvatar(currentUserAvatar) }) }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to add answer.'); await loadCommunityPosts(); } catch (error: any) { Alert.alert('Answer Failed', error?.message || 'Unable to post answer.'); } };
  const handleEditCommunityPost = async (postId: string, content: string) => { const trimmedContent = content.trim(); if (!trimmedContent) return; try { const response = await apiFetch(`${API_BASE_URL}/community-posts/${postId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: trimmedContent }) }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to update post.'); await loadCommunityPosts(); } catch (error: any) { Alert.alert('Update Failed', error?.message || 'Unable to update post.'); } };
  const handleDeleteCommunityPost = async (postId: string) => { try { const response = await apiFetch(`${API_BASE_URL}/community-posts/${postId}`, { method: 'DELETE' }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to delete post.'); await loadCommunityPosts(); } catch (error: any) { Alert.alert('Delete Failed', error?.message || 'Unable to delete post.'); } };
  const handleEditCommunityAnswer = async (postId: string, answerId: string, message: string) => { const trimmedMessage = message.trim(); if (!trimmedMessage) return; try { const response = await apiFetch(`${API_BASE_URL}/community-posts/${postId}/answers/${answerId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: trimmedMessage }) }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to update answer.'); await loadCommunityPosts(); } catch (error: any) { Alert.alert('Update Failed', error?.message || 'Unable to update answer.'); } };
  const handleDeleteCommunityAnswer = async (postId: string, answerId: string) => { try { const response = await apiFetch(`${API_BASE_URL}/community-posts/${postId}/answers/${answerId}`, { method: 'DELETE' }); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to delete answer.'); await loadCommunityPosts(); } catch (error: any) { Alert.alert('Delete Failed', error?.message || 'Unable to delete answer.'); } };
  useEffect(() => { loadCommunityPosts(); }, []);

  const getScorePercent = (assignment: { status: 'pending' | 'submitted' | 'graded'; points?: number; maxPoints?: number }) => { if (assignment.status !== 'graded' || assignment.points === undefined || assignment.maxPoints === undefined || assignment.maxPoints === 0) return null; return Math.round((assignment.points / assignment.maxPoints) * 100); };
  ...
  const buildGeneratedActivity = (course: CourseDetailData, assignment: DashboardAssignment | CourseAssignment | AssignmentItem): GenerateActivityData | null => {
    ...
    return { courseId: course.id, courseName: course.name, courseCode: course.code, assignmentId: assignment.id, assignmentTitle: assignment.title, topic: assignment.topic || assignment.title, score, recommendationType, difficulty, instructions, basedOnMaterials: course.materials.filter((material) => assignment.materialIds?.includes(material.id)).map((material) => material.title) };
  };

  const loadCompletedActivityScores = useCallback(async () => { if (!currentStudent?.studentId) { setCompletedActivityScores({}); return; } try { const response = await apiFetch(`${API_BASE_URL}/student-activities/completed-scores/${encodeURIComponent(currentStudent.studentId)}`); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to load completed support activities.'); setCompletedActivityScores(data?.data?.scores || {}); } catch (error) { console.log('LOAD COMPLETED ACTIVITY SCORES ERROR =>', error); } }, [currentStudent?.studentId]);
  useEffect(() => { void loadCompletedActivityScores(); }, [loadCompletedActivityScores]);

  const loadStudentNotifications = useCallback(async () => { if (!currentStudent?.studentId) { setStudentNotifications([]); return; } try { const response = await apiFetch(`${API_BASE_URL}/notifications?userId=${encodeURIComponent(currentStudent.studentId)}&role=student`); const data = await response.json(); if (!response.ok) throw new Error(data?.error || 'Failed to load notifications.'); setStudentNotifications(Array.isArray(data?.data) ? data.data : []); } catch (error) { console.log('LOAD STUDENT NOTIFICATIONS ERROR =>', error); } }, [currentStudent?.studentId]);
  ...
  const adaptiveVideoRecommendation = useMemo(() => {
    const weakRecommendations = joinedAssignmentCourses.flatMap((course) => course.assignments.map((assignment) => { const score = getScorePercent(assignment); if (score === null || score >= 75) return null; const completedSupportActivity = completedActivityScores[assignment.id]; if (!!completedSupportActivity?.completed && completedSupportActivity.scorePercent !== null && completedSupportActivity.scorePercent >= 75) return null; const relatedMaterialTitles = (course.materials || []).filter((material) => assignment.materialIds?.includes(material.id)).map((material) => cleanVideoSearchText(material.title)).filter(Boolean); const primaryMaterial = relatedMaterialTitles[getRotatingIndex(relatedMaterialTitles.length, assignment.id.length)] || cleanVideoSearchText(assignment.topic || '') || cleanVideoSearchText(course.name); if (!primaryMaterial) return null; return { type: 'weak' as const, primaryTopic: primaryMaterial, score, query: `${primaryMaterial}`.trim(), reason: relatedMaterialTitles.length > 0 ? `Related material (${primaryMaterial})` : `Weak topic (${primaryMaterial})` }; }).filter(Boolean)).sort((a: any, b: any) => (a.score ?? 100) - (b.score ?? 100)) as Array<{ type: 'weak'; primaryTopic: string; score: number; query: string; reason: string }>;
    const courseRecommendations = joinedAssignmentCourses.map((course, courseIndex) => { const materialTitles = (course.materials || []).map((material) => cleanVideoSearchText(material.title)).filter(Boolean); const rotatingMaterial = materialTitles[getRotatingIndex(materialTitles.length, courseIndex)] || cleanVideoSearchText(course.name); if (!rotatingMaterial) return null; return { type: 'course' as const, primaryTopic: rotatingMaterial, query: `${rotatingMaterial} tutorial lesson explanation`.trim(), reason: `Course topic (${course.name})` }; }).filter(Boolean) as Array<{ type: 'course'; primaryTopic: string; query: string; reason: string }>;
    if (weakRecommendations.length > 0) { const selectedWeak = weakRecommendations[0]; return { query: selectedWeak.query, reason: `${selectedWeak.reason}`, rotationKey: selectedWeak.query.length + selectedWeak.score }; }
    if (courseRecommendations.length > 0) { const selectedCourse = courseRecommendations[getRotatingIndex(courseRecommendations.length, 5)]; return { query: selectedCourse.query, reason: selectedCourse.reason, rotationKey: getRotatingIndex(1000, 5) }; }
  ...
  const buildLocalMaterialFallbackActivity = ({ course, assignment, score, relatedMaterials }: { course: CourseDetailData; assignment: DashboardAssignment | CourseAssignment | AssignmentItem; score: number; relatedMaterials: CourseDetailData['materials'] }): GenerateActivityData => {
    ...
    return { courseId: course.id, courseName: course.name, courseCode: course.code, assignmentId: assignment.id, assignmentTitle: assignment.title, topic, score, recommendationType, difficulty, instructions: recommendationType === 'review' ? 'Review the related lesson material carefully, answer the quick check, and explain the concept in your own words.' : 'Complete this guided practice based on the related lesson material to improve your understanding.', basedOnMaterials: materialTitles, quiz: null };
  };
  ...
  const openGeneratedActivity = async (course: CourseDetailData, assignment: DashboardAssignment | CourseAssignment | AssignmentItem) => {
    ...
    if (completedSupportActivity?.completed && completedSupportActivity.scorePercent !== null && completedSupportActivity.scorePercent >= 75) { Alert.alert('Already mastered', `You already scored ${completedSupportActivity.scorePercent}% on the generated follow-up activity for this assignment.`); return; }
    ...
    try {
      ...
      await loadStudentNotifications(); setGeneratedActivity(activity); setActiveScreen('generateactivity');
    } catch (error: any) { console.log('GENERATE ACTIVITY ERROR =>', error); Alert.alert('Generate Activity Failed', error?.message || 'The AI material scan failed. Please try again after confirming the related material file is readable.'); }
    finally { setIsGeneratingActivity(false); }
  };
  ...
  const handlePlayGame = async (assignment: any) => {
    ...
    try {
      ...
      const questions = (data.questions || []).map((q: any, idx: number) => ({ id: q.id || `q-${idx}`, question: q.question, options: q.options, answer: q.answer, correctIndex: q.options ? q.options.indexOf(q.answer) : 0 }));
      setGameAssignmentData({ assignmentId: assignment.id, assignmentTitle: assignment.title || assignment.header || 'Game Assignment', gameType: data.gameType || assignment.gameType || 'quiz_master', questions, timeLimit: data.timeLimit || assignment.timeLimit || null, customTimeLimit: data.customTimeLimit || assignment.customTimeLimit || null, numberOfAttempts: data.numberOfAttempts || assignment.numberOfAttempts || '1' });
      setLastScreen(activeScreen); setActiveScreen('gamebasedassignment');
    } catch (error: any) { Alert.alert('Game Error', error?.message || 'Unable to start the game.'); }
  ...
  const renderScreen = () => {
    switch (activeScreen) {
      ...
      case 'flipit': return <FlipIt onBack={exitFullscreenGameToGames} />;
      case 'fruitmania': return <FruitMania onBack={exitFullscreenGameToGames} />;
  ...
  return (
    <>
      <StatusBar hidden={isFullscreenScreen} translucent={isFullscreenScreen} backgroundColor={isFullscreenScreen ? 'transparent' : '#fff'} barStyle="dark-content" />
      <SafeAreaView style={[styles.safeArea, isFullscreenScreen && styles.safeAreaFullscreen, isMobileFullscreenScreen && styles.safeAreaMobileFullscreen]} edges={safeAreaEdges}>
        {shouldShowHeader && (
        ...
        {!isFullscreenScreen && !isMobileFullscreenScreen && !isLargeScreen && isMobileDrawerOpen && (<View style={styles.mobileDrawerPortal}><Pressable style={styles.mobileBackdrop} onPress={() => setMobileDrawerOpen(false)} /><View style={[styles.mobileOverlay, hasImageChanged && { paddingTop: insets.top, paddingBottom: insets.bottom, paddingLeft: insets.left, paddingRight: insets.right }]}><DrawerMenu isFixed={false} onClose={() => setMobileDrawerOpen(false)} activeScreen={activeScreen} onNavigate={handleNavigate} userName={currentUserName} userEmail={currentUserEmail} userAvatar={currentUserAvatar} userId={currentStudent.studentId} userRole="student" apiBaseUrl={API_BASE_URL} onEmailUpdated={handleDrawerEmailUpdated} onAvatarPress={() => { setMobileDrawerOpen(false); handleNavigate('profile'); }} setIsLoggedIn={() => onLogout()} /></View></View>)}
        <AnnouncementModal visible={!isFullscreenScreen && activeScreen === 'home' && showAnnouncement && studentAnnouncements.length > 0} onClose={() => setShowAnnouncement(false)} announcements={studentAnnouncements} />
        {!isFullscreenScreen && activeScreen !== 'messenger' && activeScreen !== 'notification' && activeScreen !== 'coursedetail' && activeScreen !== 'generateactivity' && !(activeScreen === 'videos' && isVideoActive) && (<Pressable style={[styles.floatingChatBtn, !isLargeScreen && styles.floatingChatBtnSmall, { bottom: hasImageChanged ? insets.bottom + 12 : 12, right: hasImageChanged ? insets.right + 20 : 20 }]} onPress={() => setIsChatOpen((prev) => !prev)}>{isChatOpen ? <Text style={[styles.chatClose, { color: '#fff' }]}>✕</Text> : <><Image source={require('../assets/images/AI.png')} style={styles.chatBtnImage} />{isLargeScreen && <Text style={styles.chatBtnLabel}>Ask anything</Text>}</>}</Pressable>)}
        {!isFullscreenScreen && <GeminiFloatingModal visible={isChatOpen} onClose={() => setIsChatOpen(false)} currentStudentId={currentStudent.studentId} />}
        {isGeneratingActivity && (<View style={styles.generatingOverlay} pointerEvents="auto"><View style={styles.generatingCard}><ActivityIndicator size="large" color="#D32F2F" /><Text style={styles.generatingTitle}>Generating activity...</Text><Text style={styles.generatingText}>AI is reading the related material file and creating 10 mixed quiz/activity questions.</Text></View></View>)}
      </SafeAreaView>
    </>
  );
}
