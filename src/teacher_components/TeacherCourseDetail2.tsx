...
const parseDueDateTime = (value?: string) => {
  ...
  if (dateOnly) return new Date(Number(dateOnly[1]), Number(dateOnly[2]) - 1, Number(dateOnly[3]), 23, 59);
  return new Date();
};
...
const mapMaterial = (item: any): Material => ({
  id: item.id, title: item.title || '', week: item.week || '', posted: formatDateTime(item.createdAt || item.posted),
  content: item.content || '', fileName: item.fileName || undefined, fileUri: item.fileUrl || item.fileUri || undefined, fileType: item.fileType || undefined,
});

const mapAssignment = (item: any): Assignment => ({
  id: item.id, header: item.header || '', instruction: item.instruction || '', posted: formatDateTime(item.createdAt || item.posted),
  dueDate: item.dueDate || '', totalScore: String(item.totalScore ?? ''), pointsOnTime: String(item.pointsOnTime ?? ''),
  repositoryDisabledAfterDue: !!item.repositoryDisabledAfterDue, materialIds: Array.isArray(item.materialIds) ? item.materialIds : [],
  fileName: item.fileName || undefined, fileUri: item.fileUrl || item.fileUri || undefined, fileType: item.fileType || undefined,
  assignmentType: item.assignmentType || 'regular', gameType: item.gameType, numberOfAttempts: item.numberOfAttempts,
  customAttempts: item.customAttempts, timeLimit: item.timeLimit, customTimeLimit: item.customTimeLimit, questions: item.questions || [],
});

const mapMember = (item: any): Member => ({
  id: item.userId || item.id || '', name: item.name || '', handle: item.email ? `@${String(item.email).split('@')[0]}` : '@member',
});

const mapSubmission = (item: any): Submission => ({
  id: item.id, assignmentId: item.assignmentId || '', studentId: item.studentId || '', studentName: item.studentName || '',
  status: item.status === 'submitted' || item.status === 'graded' || item.status === 'late' ? item.status : 'pending',
  score: typeof item.score === 'number' ? item.score : item.score === null ? undefined : Number(item.score),
  submittedAt: formatDateTime(item.submittedAt), fileName: item.fileName || undefined, fileUrl: item.fileUrl || undefined,
  fileType: item.fileType || undefined, feedback: item.feedback || '',
});

const TeacherCourseDetail2 = ({
  onBack, course, currentTeacher, availableCourses = [],
}: {
  onBack?: () => void; course?: CourseDetailData; currentTeacher: SignedInTeacher; availableCourses?: CourseDetailData[];
}) => {
  ...
  const teacherIdentity = useMemo(() => {
    return currentTeacher?.teacherId?.trim() || currentTeacher?.authUid?.trim() || currentTeacher?.email?.trim() || teacherFullName;
  ...
  const [gameType, setGameType] = useState<'quiz_master' | 'memory_match' | 'fill_in_blanks' | 'flashcard' | 'boss_battle' | ''>('');
  ...
  const showResultModal = (type: 'success' | 'error', title: string, message: string) => {
    setResultModalType(type); setResultModalTitle(title); setResultModalMessage(message); setResultModalVisible(true);
  };

  const loadCourseContent = async () => {
    if (!course?.id) { setAssignments([]); setMaterials([]); setMembers([]); setSubmissions([]); return; }
    try {
      ...
      const [materialsData, assignmentsData, membersData, submissionsData] = await Promise.all([
        materialsRes.json(), assignmentsRes.json(), membersRes.json(), submissionsRes.json(),
      ]);
      ...
      setMembers(membersRes.ok && Array.isArray(membersData) ? membersData.filter((item: any) => item?.role === 'student').map(mapMember) : []);
      setSubmissions(submissionsRes.ok && Array.isArray(submissionsData) ? submissionsData.map(mapSubmission) : []);
    } catch (error) {
      console.error('Error loading course content:', error);
      setAssignments([]); setMaterials([]); setMembers([]); setSubmissions([]);
    }
  };

  useEffect(() => { loadCourseContent(); }, [course?.id]);

  const resetCreateForm = () => {
    setFormTitle(''); setFormDesc(''); setFormPoints(''); setFormDue(''); setFormPointsOnTime('');
    setSelectedMaterialIds([]); setAssignmentDisableRepositoryAfterDue(false); setPickedFile(null); setPickedAssignmentFile(null); setErrors({});
    const now = new Date(); setDraftDueDateTime(now); setVisibleCalendarMonth(new Date(now.getFullYear(), now.getMonth(), 1));
    setAssignmentType('regular'); setGameType(''); setSelectedClassId(course?.id || '');
    setNumberOfAttempts('1'); setCustomAttempts(''); setTimeLimit(''); setCustomTimeLimit('');
    setGeneratedQuestions([]); setShowGeneratedPreview(false);
    setShowGameTypeModal(false); setShowClassModal(false);
  };
  ...
  const openUpdateModal = (item: Assignment | undefined) => {
    if (!item) return;
    setSelectedId(item.id); setFormTitle(item.header); setFormDesc(item.instruction); setFormPoints(item.totalScore);
    setFormDue(item.dueDate); setFormPointsOnTime(item.pointsOnTime); setSelectedMaterialIds(item.materialIds || []);
    setAssignmentDisableRepositoryAfterDue(item.repositoryDisabledAfterDue); setErrors({});
    const parsed = parseDueDateTime(item.dueDate); setDraftDueDateTime(parsed); setVisibleCalendarMonth(new Date(parsed.getFullYear(), parsed.getMonth(), 1));
    setAssignmentType((item as any).assignmentType || 'regular'); setGameType((item as any).gameType || '');
  ...
  const openMaterialModal = (material: Material) => { setSelectedMaterial(material); setShowMaterialModal(true); };
  const openDateTimePicker = () => {
    const parsed = parseDueDateTime(formDue); setDraftDueDateTime(parsed); setVisibleCalendarMonth(new Date(parsed.getFullYear(), parsed.getMonth(), 1)); setShowDateTimeModal(true);
  };
  const applyDraftDateTime = () => {
    if (draftDueDateTime.getTime() < startOfToday().getTime()) { setErrors((prev) => ({ ...prev, dueDate: 'Past dates are not allowed.' })); return; }
    setFormDue(formatDueDateTime(draftDueDateTime)); setErrors((prev) => ({ ...prev, dueDate: undefined })); setShowDateTimeModal(false);
  };
  const selectDraftDate = (date: Date) => {
    const next = new Date(draftDueDateTime); next.setFullYear(date.getFullYear(), date.getMonth(), date.getDate()); setDraftDueDateTime(next);
  };
  const updateDraftTime = (field: 'hours' | 'minutes', value: number) => {
    const next = new Date(draftDueDateTime);
    if (field === 'hours') next.setHours(value); if (field === 'minutes') next.setMinutes(value); setDraftDueDateTime(next);
  };
  ...
  const handlePickFile = async () => {
    try {
      ...
      setPickedFile({ name: asset.name, uri: asset.uri, type: asset.mimeType, base64: (asset as any).base64, file: (asset as any).file });
    } catch { showResultModal('error', 'Error', 'Failed to pick file.'); }
  };
  ...
  const handlePickAssignmentFile = async () => {
    try {
      ...
      if (result.canceled || !result.assets?.[0]) return;
      const asset = result.assets[0];
      setPickedAssignmentFile({ name: asset.name, uri: asset.uri, type: asset.mimeType, base64: (asset as any).base64, file: (asset as any).file });
    } catch { showResultModal('error', 'Error', 'Failed to pick assignment file.'); }
  };
  ...
  const uploadPickedFile = async (picked: PickedUploadFile, kind: 'material' | 'assignment') => {
    ...
    if (Platform.OS === 'web') {
      if (picked.base64) fileBase64 = picked.base64;
      else if (picked.file) {
    ...
    const response = await fetch(`${API_BASE_URL}/upload-class-file`, {
      ...
      body: JSON.stringify({ classId: course.id, fileBase64, fileName: picked.name ?? 'file', fileType: picked.type ?? 'application/octet-stream', kind }),
    });
  ...
  const handleOpenUploadedFile = async (fileUri?: string) => {
    if (!fileUri) { showResultModal('error', 'No File', 'No uploaded file available.'); return; }
    try { await Linking.openURL(fileUri); } catch { showResultModal('error', 'Error', 'Unable to open the file.'); }
  };
  ...
  const handleGradeSubmission = async (submissionId: string, score: number, feedback?: string) => {
    ...
    } catch (error: any) { showResultModal('error', 'Grade Failed', error?.message || 'Unable to save score.'); }
  };

  const handleCopyClassCode = async () => {
    const codeToCopy = course?.classCode || '';
    if (!codeToCopy || codeToCopy === 'No Class Code') return;
    await Clipboard.setStringAsync(codeToCopy); setClassCodeCopied(true); setTimeout(() => setClassCodeCopied(false), 2000);
  };
  ...
  const validateAssignmentForm = () => {
    ...
    if (assignmentType === 'game_based') {
      ...
      if (availableCourses.length > 1 && !selectedClassId) nextErrors.classId = 'Please select a class.';
      if (!numberOfAttempts) nextErrors.attempts = 'Please select number of attempts.';
      else if (numberOfAttempts === 'custom' && (!customAttempts.trim() || Number(customAttempts) < 1)) nextErrors.customAttempts = 'Custom attempts must be at least 1.';
      if (!timeLimit) nextErrors.timeLimit = 'Please select a time limit.';
      if (timeLimit === 'custom' && !customTimeLimit.trim()) nextErrors.customTimeLimit = 'Please enter custom time limit.';
    }
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length > 0) { showResultModal('error', 'Required', 'Please complete the highlighted assignment fields.'); return false; }
    return true;
  };
  ...
  const handleCreate = async () => {
    if (isSaving) return;
    if (!course?.id) { showResultModal('error', 'Error', 'No class selected.'); return; }

    if (activeTab === 'Materials') {
      if (!formTitle.trim() || !formPointsOnTime.trim()) { showResultModal('error', 'Required', 'Please enter the title and week.'); return; }
      setIsSaving(true);
      try {
        let uploadedFile = null;
        if (pickedFile?.uri || pickedFile?.base64 || pickedFile?.file) uploadedFile = await uploadPickedFile(pickedFile, 'material');
        const response = await fetch(`${API_BASE_URL}/create-class-material`, {
          credentials: 'include', method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            classId: course.id, title: formTitle.trim(), week: formPointsOnTime.trim(), content: formDesc.trim() || `${formPointsOnTime.trim()} material: ${formTitle.trim()}`,
        ...
        await loadCourseContent(); setShowCreateModal(false); resetCreateForm(); showResultModal('success', 'Success', 'Material uploaded successfully.');
      } catch (error: any) { showResultModal('error', 'Upload Failed', error?.message || 'Failed to create material.'); } finally { setIsSaving(false); }
      return;
    }
    ...
    try {
      let uploadedFile = null;
      if (pickedAssignmentFile?.uri || pickedAssignmentFile?.base64 || pickedAssignmentFile?.file) uploadedFile = await uploadPickedFile(pickedAssignmentFile, 'assignment');
      ...
      await loadCourseContent(); setShowCreateModal(false); resetCreateForm(); showResultModal('success', 'Success', 'Assignment uploaded successfully.');
    } catch (error: any) { showResultModal('error', 'Upload Failed', error?.message || 'Failed to create assignment.'); } finally { setIsSaving(false); }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await loadCourseContent(); setShowUpdateModal(false); showResultModal('success', 'Success', 'Assignment updated.');
    } catch (error: any) { showResultModal('error', 'Error', error?.message || 'Failed to update assignment.'); }
  };

  const handleDelete = async () => {
    if (!selectedId) return;
    try {
      const response = await fetch(`${API_BASE_URL}/delete-class-assignment/${selectedId}`, { credentials: 'include', method: 'DELETE' });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Failed to delete assignment');
      await loadCourseContent(); setShowUpdateModal(false); setShowSubmissions(false); showResultModal('success', 'Success', 'Assignment deleted.');
    } catch (error: any) { showResultModal('error', 'Error', error?.message || 'Failed to delete assignment.'); }
  };
  ...
  const renderInputError = (message?: string) => !!message ? <Text style={styles.errorText}>{message}</Text> : null;

  const renderRelatedMaterialsSelector = () => (
    <View style={styles.sectionBlock}>
      <Text style={styles.sectionLabel}>Learning Materials</Text>
      <Text style={styles.helperText}>Select the materials the AI should use for follow-up activity generation or game content.</Text>
      {materials.length === 0 ? (
        <Text style={styles.emptyMiniText}>No created materials yet.</Text>
      ) : (
        <View style={[styles.materialSelectorWrap, errors.materials ? styles.errorContainer : null]}>
          {materials.map((material) => {
            const active = selectedMaterialIds.includes(material.id);
            return (
              <TouchableOpacity key={material.id} style={[styles.materialChip, active && styles.materialChipActive]} onPress={() => toggleRelatedMaterial(material.id)} activeOpacity={0.85} disabled={isSaving}>
                <Ionicons name={active ? 'checkmark-circle' : 'ellipse-outline'} size={16} color={active ? '#FFF' : '#D32F2F'} />
                <Text style={[styles.materialChipText, active && styles.materialChipTextActive]}>{material.title}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
  ...
  const renderDateTimeField = () => (
    <View style={styles.sectionBlock}>
      ...
      <TouchableOpacity style={[styles.dateButton, errors.dueDate ? styles.errorBorder : null, isSaving ? styles.disabledInput : null]} onPress={openDateTimePicker} activeOpacity={0.85} disabled={isSaving}>
    ...
    return (
      <View style={[styles.gameAndClassRow, isMobile && styles.gameAndClassRowMobile]}>
        <View style={[styles.dropdownWrap, !isMobile && styles.dropdownWrapHalf]}>
          ...
          <TouchableOpacity style={[styles.dropdownTrigger, errors.gameType ? styles.errorBorder : null]} onPress={() => setShowGameTypeModal(true)} disabled={isSaving} activeOpacity={0.8}>
            <Text style={[styles.dropdownText, !selectedGame && styles.dropdownPlaceholder]} numberOfLines={1}>{selectedGame ? selectedGame.label : 'Choose a game type'}</Text>
        ...
        {availableCourses.length > 1 && (
          <View style={[styles.dropdownWrap, !isMobile && styles.dropdownWrapHalf]}>
            ...
            <TouchableOpacity style={[styles.dropdownTrigger, errors.classId ? styles.errorBorder : null]} onPress={() => setShowClassModal(true)} disabled={isSaving} activeOpacity={0.8}>
              <Text style={[styles.dropdownText, !selectedClass && styles.dropdownPlaceholder]} numberOfLines={1}>{selectedClass ? `${selectedClass.courseCode} - ${selectedClass.name}` : 'Select a class'}</Text>
              <Ionicons name="chevron-down" size={18} color="#D32F2F" />
            </TouchableOpacity>
    ...
    <View style={styles.sectionBlock}>
      ...
      <View style={{ flexDirection: 'row', gap: 10, marginTop: 8, flexWrap: 'wrap' }}>
        {['1', '2', '3', 'unlimited', 'custom'].map((val) => (
          <TouchableOpacity key={val} style={[styles.attemptChip, numberOfAttempts === val && styles.attemptChipActive]} onPress={() => {
            setNumberOfAttempts(val); if (val !== 'custom') setCustomAttempts('');
            if (errors.attempts) setErrors((prev) => ({ ...prev, attempts: undefined }));
            if (errors.customAttempts) setErrors((prev) => ({ ...prev, customAttempts: undefined }));
          }} disabled={isSaving}>
            <Text style={[styles.attemptChipText, numberOfAttempts === val && styles.attemptChipTextActive]}>
      ...
      {numberOfAttempts === 'custom' && (
        <TextInput style={[styles.inputBox, { marginTop: 10 }, errors.customAttempts ? styles.errorBorder : null]} value={customAttempts} onChangeText={(val) => {
          setCustomAttempts(val.replace(/[^0-9]/g, '')); if (errors.customAttempts) setErrors((prev) => ({ ...prev, customAttempts: undefined }));
        }} keyboardType="numeric" placeholder="Enter number (e.g., 5)" placeholderTextColor="#999" editable={!isSaving} />
      )}
    ...
    <View style={[styles.formGrid, !isMobile && styles.formGridDesktop]}>
      <View style={styles.sectionBlock}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginTop: 8 }}>
          <TouchableOpacity style={[styles.typeChip, assignmentType === 'regular' && styles.typeChipActive]} onPress={() => setAssignmentType('regular')} disabled={isSaving}>
            <Text style={[styles.typeChipText, assignmentType === 'regular' && styles.typeChipTextActive]}>Regular Submission</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.typeChip, assignmentType === 'game_based' && styles.typeChipActive]} onPress={() => setAssignmentType('game_based')} disabled={isSaving}>
            <Text style={[styles.typeChipText, assignmentType === 'game_based' && styles.typeChipTextActive]}>Game Based Assignment</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={[styles.formColumnLeft, !isMobile && styles.formColumnLeftDesktop]}>
        ...
        <TextInput style={[styles.inputBox, errors.title ? styles.errorBorder : null]} value={formTitle} onChangeText={(value) => {
          setFormTitle(value); if (errors.title) setErrors((prev) => ({ ...prev, title: undefined }));
        }} placeholder="Enter Header" placeholderTextColor="#999" editable={!isSaving} />
        {renderInputError(errors.title)}

        <Text style={styles.sectionLabel}>Instruction</Text>
        <TextInput style={[styles.textAreaBox, errors.instruction ? styles.errorBorder : null]} value={formDesc} onChangeText={(value) => {
          setFormDesc(value); if (errors.instruction) setErrors((prev) => ({ ...prev, instruction: undefined }));
        }} placeholder="Enter Instruction" placeholderTextColor="#999" multiline editable={!isSaving} />
      ...
      <View style={styles.fullWidthSection}>
        ...
        {assignmentType === 'regular' && (
          <>
            ...
            <TouchableOpacity style={[styles.primaryButtonWide, isSaving ? styles.disabledButton : null]} onPress={handlePickAssignmentFile} disabled={isSaving}>
              <Ionicons name="cloud-upload-outline" size={18} color="#FFF" />
              <Text style={styles.uploadBtnText}>{pickedAssignmentFile?.name ? 'Change File' : 'Upload File'}</Text>
            </TouchableOpacity>
        ...
        <View style={styles.checkboxRow}>
          <TouchableOpacity style={[styles.checkboxBox, assignmentDisableRepositoryAfterDue && styles.checkboxBoxChecked]} onPress={() => setAssignmentDisableRepositoryAfterDue(!assignmentDisableRepositoryAfterDue)}>
            {assignmentDisableRepositoryAfterDue ? <Ionicons name="checkmark" size={16} color="#FFF" /> : null}
          </TouchableOpacity>
    ...
    if (activeTab === 'Materials') {
      return (
        <>
          ...
          <TextInput style={styles.inputBox} placeholder="Material Title" placeholderTextColor="#999" value={formTitle} onChangeText={setFormTitle} />
          <Text style={styles.sectionLabel}>Week</Text>
          <TextInput style={styles.inputBox} placeholder="Week (example: Week 1)" placeholderTextColor="#999" value={formPointsOnTime} onChangeText={setFormPointsOnTime} />
          <Text style={styles.sectionLabel}>Description</Text>
          <TextInput style={styles.textAreaBox} placeholder="Optional description" placeholderTextColor="#999" value={formDesc} onChangeText={setFormDesc} multiline />
          <Text style={styles.sectionLabel}>Attachment</Text>
          <TouchableOpacity style={[styles.primaryButtonWide, isSaving ? styles.disabledButton : null]} onPress={handlePickFile} disabled={isSaving}>
            <Ionicons name="cloud-upload-outline" size={18} color="#FFF" />
  ...
  if (showSubmissions) {
    return (
      <>
        <TeacherSubmissionsSection members={members} currentAssignment={selectedAssignment} submissions={submissions} onBack={() => setShowSubmissions(false)} onOpenUpdate={() => openUpdateModal(selectedAssignment)} onGradeSubmission={handleGradeSubmission} />
          ...
          <View style={styles.modalOverlayCenter}>
            <View style={[styles.modalCardElevated, { width: isMobile ? Math.min(width - 28, 360) : 820, maxHeight: height * 0.9 }]}>
              <View style={styles.createHeaderRow}>
                ...
                <TouchableOpacity onPress={() => setShowUpdateModal(false)}><Ionicons name="close" size={24} color="#111" /></TouchableOpacity>
              </View>
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalScrollContent}>{renderAssignmentFields()}</ScrollView>
              <View style={styles.modalBottomActions}>
                <TouchableOpacity style={styles.secondaryButton} onPress={handleDelete}><Text style={styles.secondaryButtonText}>Delete</Text></TouchableOpacity>
                <TouchableOpacity style={styles.primaryButton} onPress={handleUpdate}><Text style={styles.primaryButtonText}>Update</Text></TouchableOpacity>
              </View>
            </View>
          </View>
        </Modal>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.screenScroll} contentContainerStyle={styles.screenScrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.courseHeader, { paddingHorizontal: isMobile ? 16 : 60, paddingTop: isMobile ? 16 : 20, paddingBottom: isMobile ? 18 : 24 }]}>
          ...
          <View style={styles.headerTopRow}>
            <Text style={[styles.courseName, { fontSize: isSmallPhone ? 20 : 24, flex: 1 }]} numberOfLines={1}>{courseName}</Text>
            <TouchableOpacity style={styles.exportGradesButtonInline} onPress={downloadClassGradesExcel} activeOpacity={0.85}>
              <Ionicons name="download-outline" size={18} color="#FFFFFF" />
              {!isMobile && <Text style={styles.exportGradesButtonText}>Export Grades Excel</Text>}
            </TouchableOpacity>
          </View>
          <View style={styles.headerInfoCard}>
            <View style={styles.headerInfoRow}>
              <Text style={styles.headerInfoLabel}>INSTRUCTOR</Text>
              <Text style={styles.headerInfoValue} numberOfLines={1}>{courseInstructor}</Text>
            </View>
            <View style={[styles.headerDetailsGrid, !isMobile && styles.headerDetailsGridDesktop]}>
              {!!courseYear && (
                <View style={[styles.academicInfoPill, !isMobile && styles.headerDetailItemDesktop]}>
                  ...
                  <View style={styles.academicInfoTextWrap}><Text style={styles.academicInfoLabel}>Year</Text><Text style={styles.academicInfoValue} numberOfLines={1}>{courseYear}</Text></View>
                </View>
              )}
              {!!courseSection && (
                <View style={[styles.academicInfoPill, !isMobile && styles.headerDetailItemDesktop]}>
                  <Ionicons name="people-outline" size={14} color="#D32F2F" />
                  <View style={styles.academicInfoTextWrap}><Text style={styles.academicInfoLabel}>Section</Text><Text style={styles.academicInfoValue} numberOfLines={1}>{courseSection}</Text></View>
                </View>
              )}
              {!!courseSemester && (
                <View style={[styles.academicInfoPill, !isMobile && styles.headerDetailItemDesktop]}>
                  <Ionicons name="calendar-outline" size={14} color="#D32F2F" />
                  <View style={styles.academicInfoTextWrap}><Text style={styles.academicInfoLabel}>Semester</Text><Text style={styles.academicInfoValue} numberOfLines={1}>{courseSemester}</Text></View>
                </View>
              )}
              {!!schoolYear && (
                <View style={[styles.academicInfoPill, !isMobile && styles.headerDetailItemDesktop]}>
                  <Ionicons name="time-outline" size={14} color="#D32F2F" />
                  <View style={styles.academicInfoTextWrap}><Text style={styles.academicInfoLabel}>School Year</Text><Text style={styles.academicInfoValue} numberOfLines={1}>{schoolYear}</Text></View>
                </View>
              )}
              <View style={[styles.classCodeBox, !isMobile && styles.headerDetailItemDesktop]}>
                <Ionicons name="copy-outline" size={14} color="#D32F2F" />
                <View style={styles.classCodeTextWrap}><Text style={styles.classCodeLabel}>CLASS CODE</Text><Text style={styles.classCodeValue} numberOfLines={1}>{classCode}</Text></View>
                <TouchableOpacity style={styles.copyCodeButton} onPress={handleCopyClassCode}>
                  <Ionicons name={classCodeCopied ? 'checkmark-outline' : 'copy-outline'} size={15} color="#D32F2F" />
        ...
        <View style={styles.tabContainer}>
          <TouchableOpacity onPress={() => setActiveTab('Materials')} style={[styles.tab, activeTab === 'Materials' && styles.tabActive]}>
            <Text style={[styles.tabText, activeTab === 'Materials' && styles.tabTextActive]}>Materials ({materials.length})</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => setActiveTab('Assignments')} style={[styles.tab, activeTab === 'Assignments' && styles.tabActive]}>
            <Text style={[styles.tabText, activeTab === 'Assignments' && styles.tabTextActive]}>Assignments ({assignments.length})</Text>
          </TouchableOpacity>
        </View>
        ...
        {activeTab === 'Materials' ? (
          ...
          <TeacherAssignmentSection assignments={assignments} onCreate={openCreateModal} onOpenMembers={(id) => { setSelectedId(id); setShowSubmissions(true); }} />
        )}
      </ScrollView>
      ...
      <Modal visible={showCreateModal} transparent animationType="fade">
        <View style={styles.modalOverlayCenter}>
          <View style={[styles.modalCardElevated, { width: isMobile ? Math.min(width - 28, 370) : 900, maxHeight: height * 0.9 }]}>
            <View style={styles.createHeaderRow}>
              <View style={styles.modalHeaderTextWrap}>
                <Text style={styles.createTitle}>Create {activeTab === 'Materials' ? 'Material' : (assignmentType === 'game_based' ? 'Game-Based Assignment' : 'Assignment')}</Text>
                <Text style={styles.modalSubtitle}>
                  {activeTab === 'Materials' ? 'Add a new class material with optional file attachment.' : assignmentType === 'game_based' ? 'Create a new game-based assignment with interactive challenges.' : 'Create a new assignment with professional responsive layout, aligned date and time, related materials, and optional file upload.'}
                </Text>
              </View>
              <TouchableOpacity onPress={() => { if (isSaving) return; setShowCreateModal(false); resetCreateForm(); }} disabled={isSaving}>
                <Ionicons name="close" size={24} color="#111" />
              </TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.modalScrollContent}>{renderCreateModalBody()}</ScrollView>
            <View style={[styles.floatingSaveWrap, isMobile && styles.floatingSaveWrapMobile]}>
              <TouchableOpacity style={[styles.floatingSaveButton, activeTab === 'Assignments' && Object.keys(errors).length > 0 ? styles.floatingSaveButtonWarn : null, isSaving ? styles.floatingSaveButtonDisabled : null]} onPress={handleCreate} disabled={isSaving} activeOpacity={isSaving ? 1 : 0.85}>
                {isSaving ? (
                  <><ActivityIndicator size="small" color="#FFF" /><Text style={styles.floatingSaveButtonText}>{activeTab === 'Materials' ? 'Saving Material...' : 'Saving Assignment...'}</Text></>
                ) : (
                  <><Ionicons name="save-outline" size={18} color="#FFF" /><Text style={styles.floatingSaveButtonText}>Save</Text></>
                )}
              </TouchableOpacity>
            </View>
          </View>
        ...
        <View style={styles.modalOverlayCenter}>
          <View style={[styles.dateTimeCard, { width: isMobile ? Math.min(width - 28, 360) : 760 }]}>
            <View style={styles.createHeaderRow}>
              ...
              <TouchableOpacity onPress={() => setShowDateTimeModal(false)}><Ionicons name="close" size={24} color="#111" /></TouchableOpacity>
            </View>
            <View style={[styles.dateTimeLayout, !isMobile && styles.dateTimeLayoutDesktop]}>
              <View style={[styles.calendarPanel, !isMobile && styles.calendarPanelDesktop]}>
                <View style={styles.calendarHeader}>
                  <TouchableOpacity style={styles.calendarNavBtn} onPress={() => setVisibleCalendarMonth(new Date(visibleCalendarMonth.getFullYear(), visibleCalendarMonth.getMonth() - 1, 1))}><Ionicons name="chevron-back" size={18} color="#D32F2F" /></TouchableOpacity>
                  <Text style={styles.calendarMonthLabel}>{monthLabel(visibleCalendarMonth)}</Text>
                  <TouchableOpacity style={styles.calendarNavBtn} onPress={() => setVisibleCalendarMonth(new Date(visibleCalendarMonth.getFullYear(), visibleCalendarMonth.getMonth() + 1, 1))}><Ionicons name="chevron-forward" size={18} color="#D32F2F" /></TouchableOpacity>
                </View>
                <View style={styles.weekRow}>{WEEKDAY_LABELS.map((label) => <Text key={label} style={styles.weekLabel}>{label}</Text>)}</View>
                <View style={styles.dayGrid}>
                  {calendarDays.map((item) => {
                    const active = isSameDate(item.date, draftDueDateTime);
                    const disabled = isPastDay(item.date);
                    return (
                      <TouchableOpacity key={item.key} style={[styles.dayCell, !item.inCurrentMonth && styles.dayCellOutside, active && styles.dayCellActive, disabled && styles.dayCellDisabled]} onPress={() => { if (disabled) return; selectDraftDate(item.date); }} disabled={disabled} activeOpacity={disabled ? 1 : 0.85}>
                        <Text style={[styles.dayText, !item.inCurrentMonth && styles.dayTextOutside, active && styles.dayTextActive, disabled && styles.dayTextDisabled]}>{item.date.getDate()}</Text>
                      </TouchableOpacity>
                    );
                  })}
                </View>
                  ...
                  <View style={styles.timeColumn}>
                    ...
                    <ScrollView style={styles.timeList} nestedScrollEnabled>
                      {Array.from({ length: 24 }, (_, hour) => (
                        <TouchableOpacity key={`hour-${hour}`} style={[styles.timeOption, draftDueDateTime.getHours() === hour && styles.timeOptionActive]} onPress={() => updateDraftTime('hours', hour)}>
                          <Text style={[styles.timeOptionText, draftDueDateTime.getHours() === hour && styles.timeOptionTextActive]}>{pad(hour)}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  </View>
                  <View style={styles.timeColumn}>
                    ...
                    <ScrollView style={styles.timeList} nestedScrollEnabled>
                      {Array.from({ length: 12 }, (_, i) => i * 5).map((minute) => (
                        <TouchableOpacity key={`minute-${minute}`} style={[styles.timeOption, draftDueDateTime.getMinutes() === minute && styles.timeOptionActive]} onPress={() => updateDraftTime('minutes', minute)}>
                          <Text style={[styles.timeOptionText, draftDueDateTime.getMinutes() === minute && styles.timeOptionTextActive]}>{pad(minute)}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  </View>
            ...
            <View style={styles.buttonRow}>
              <TouchableOpacity style={styles.secondaryButton} onPress={() => setShowDateTimeModal(false)}><Text style={styles.secondaryButtonText}>Cancel</Text></TouchableOpacity>
              <TouchableOpacity style={styles.primaryButton} onPress={applyDraftDateTime}><Text style={styles.primaryButtonText}>Apply</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
        ...
        <Pressable style={styles.modalOverlayCenter} onPress={() => setShowMaterialModal(false)}>
          <Pressable style={[styles.modalCardElevated, { width: isMobile ? Math.min(width - 28, 360) : 430 }]}>
            ...
            {!!selectedMaterial?.fileName && (
              <TouchableOpacity style={styles.openFileButton} onPress={() => handleOpenUploadedFile(selectedMaterial?.fileUri)}>
          ...
          <Pressable style={[styles.modalCardElevated, { width: isMobile ? Math.min(width - 28, 360) : 450, maxHeight: height * 0.8 }]}>
            <View style={styles.createHeaderRow}>
              <View style={styles.modalHeaderTextWrap}>
                ...
                <Text style={styles.modalSubtitle}>Choose the interactive format for this assignment.</Text>
              </View>
              <TouchableOpacity onPress={() => setShowGameTypeModal(false)}><Ionicons name="close" size={24} color="#111" /></TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled>
              {gameOptions.map((opt) => (
                <TouchableOpacity key={opt.value} style={[styles.dropdownItem, gameType === opt.value && styles.dropdownItemActive]} onPress={() => {
                  setGameType(opt.value as any); setShowGameTypeModal(false); if (errors.gameType) setErrors((prev) => ({ ...prev, gameType: undefined }));
                }}>
                  <Ionicons name={gameType === opt.value ? 'radio-button-on' : 'radio-button-off'} size={18} color={gameType === opt.value ? '#FFF' : '#D32F2F'} style={{ marginRight: 12 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.dropdownItemText, gameType === opt.value && styles.dropdownItemTextActive]}>{opt.label}</Text>
                    {!!opt.desc && <Text style={[styles.dropdownItemDesc, gameType === opt.value && styles.dropdownItemDescActive]}>{opt.desc}</Text>}
                  </View>
                </TouchableOpacity>
              ))}
            </ScrollView>
        ...
        <Pressable style={styles.modalOverlayCenter} onPress={() => setShowClassModal(false)}>
          <Pressable style={[styles.modalCardElevated, { width: isMobile ? Math.min(width - 28, 360) : 450, maxHeight: height * 0.8 }]}>
            <View style={styles.createHeaderRow}>
              ...
              <TouchableOpacity onPress={() => setShowClassModal(false)}><Ionicons name="close" size={24} color="#111" /></TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled>
              {availableCourses.map((c) => (
                <TouchableOpacity key={c.id} style={[styles.dropdownItem, selectedClassId === c.id && styles.dropdownItemActive]} onPress={() => {
                  setSelectedClassId(c.id); setShowClassModal(false); if (errors.classId) setErrors((prev) => ({ ...prev, classId: undefined }));
                }}>
                  <Ionicons name={selectedClassId === c.id ? 'radio-button-on' : 'radio-button-off'} size={18} color={selectedClassId === c.id ? '#FFF' : '#D32F2F'} style={{ marginRight: 12 }} />
                  <Text style={[styles.dropdownItemText, selectedClassId === c.id && styles.dropdownItemTextActive]} numberOfLines={1}>{c.courseCode} - {c.name}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </Pressable>
      ...
      <Modal visible={resultModalVisible} transparent animationType="fade" onRequestClose={() => setResultModalVisible(false)}>
        <View style={styles.modalOverlayCenter}>
          <View style={[styles.modalCardElevated, { width: isMobile ? Math.min(width - 28, 330) : 360 }]}>
            <Text style={[styles.createTitle, { color: resultModalType === 'success' ? '#2E7D32' : '#D32F2F' }]}>{resultModalTitle}</Text>
            <Text style={styles.previewContent}>{resultModalMessage}</Text>
            <TouchableOpacity style={styles.primaryButtonWide} onPress={() => setResultModalVisible(false)}><Text style={styles.uploadBtnText}>OK</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      {isSaving && (
        <View style={styles.savingOverlay} pointerEvents="auto">
          <View style={styles.savingCard}>
            ...
            <Text style={styles.savingTitle}>{activeTab === 'Materials' ? 'Saving Material' : 'Saving Assignment'}</Text>
            <Text style={styles.savingMessage}>Please wait while your file and details are being stored in Firebase.</Text>
          </View>
        </View>
      )}
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  courseHeader: { backgroundColor: '#D32F2F', shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 6 },
  backButton: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(255,255,255,0.12)', alignItems: 'center', justifyContent: 'center', marginBottom: 14, alignSelf: 'flex-start' },
  headerTopRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 },
  courseName: { color: '#FFF', fontWeight: '900', marginBottom: 0, letterSpacing: 0.2 },
  headerInfoCard: { backgroundColor: 'rgba(255,255,255,0.12)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)', borderRadius: 18, padding: 14 },
  headerInfoRow: { marginBottom: 12 },
  headerInfoLabel: { color: 'rgba(255,255,255,0.72)', fontSize: 10, fontWeight: '800', letterSpacing: 0.8, marginBottom: 3 },
  ...
  academicInfoPill: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#FFF', borderRadius: 14, paddingHorizontal: 10, paddingVertical: 9, minWidth: 132, flexGrow: 1, flexBasis: '100%' },
  academicInfoTextWrap: { flex: 1 },
  academicInfoLabel: { color: '#8A8A8A', fontSize: 10, fontWeight: '800', letterSpacing: 0.5, textTransform: 'uppercase' },
  academicInfoValue: { color: '#202124', fontSize: 12, fontWeight: '800', marginTop: 2 },
  classCodeBox: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFFFFF', borderRadius: 14, paddingHorizontal: 10, paddingVertical: 9, gap: 8, minWidth: 132, flexGrow: 1, flexBasis: '100%' },
  ...
  copyCodeButton: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: '#FFF4F4', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 8 },
  copyCodeText: { color: '#D32F2F', fontWeight: '900', fontSize: 12 },
  exportGradesButtonInline: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#1F7A3A', paddingHorizontal: 12, minWidth: 42, height: 42, borderRadius: 10, borderBottomWidth: 3, borderBottomColor: '#145A2A' },
  exportGradesButtonText: { color: '#FFFFFF', fontSize: 13, fontWeight: '900', letterSpacing: 0.25, textTransform: 'uppercase' },
  ...
  floatingSaveButton: { backgroundColor: '#D32F2F', borderRadius: 12, minHeight: 48, width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, shadowColor: '#000', shadowOpacity: 0.18, shadowRadius: 8, elevation: 8 },
  ...
  savingOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.38)', alignItems: 'center', justifyContent: 'center', padding: 18, zIndex: 9999, elevation: 9999 },
  savingCard: { width: '100%', maxWidth: 330, backgroundColor: '#FFFFFF', borderRadius: 18, paddingVertical: 24, paddingHorizontal: 18, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.18, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 12 },
  savingTitle: { marginTop: 12, fontSize: 17, fontWeight: '900', color: '#111', textAlign: 'center' },
  savingMessage: { marginTop: 6, fontSize: 13, lineHeight: 19, color: '#666', textAlign: 'center' },
  ...
  dropdownTrigger: { borderWidth: 1, borderColor: '#DDD', borderRadius: 12, paddingHorizontal: 14, paddingVertical: 13, backgroundColor: '#FFF', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', minHeight: 48 },
  dropdownText: { color: '#111', fontWeight: '600', fontSize: 14, flex: 1, marginRight: 8 },
  dropdownPlaceholder: { color: '#999' },
  dropdownItem: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' },
