...
// Enable LayoutAnimation for Android to ensure smooth keyboard transitions
...
const Messenger = ({
  ...
  const [imagePreviewStoragePath, setImagePreviewStoragePath] = useState<string | null>(null);
  
  // Track which images are currently being refreshed to prevent loops
  const [refreshingImages, setRefreshingImages] = useState<Set<string>>(new Set());

  // ✅ NEW: Pending File State (Facebook Messenger Style)
  ...
  // Listen to keyboard events to explicitly handle layout shifts
  useEffect(() => {
  ...
  const isSystemMessage = (item: any) => {
    return (
      ...
      item?.isSystem === true ||
      (!item?.senderName && typeof item?.text === 'string' && item.text.toLowerCase().includes('conversation created'))
    );
  };

  const resolveSenderName = (item: any) => {
    if (isSystemMessage(item)) return 'System';
    if (item?.senderName && String(item.senderName).trim()) return String(item.senderName).trim();
    if (item?.displayName && String(item.displayName).trim()) return String(item.displayName).trim();
    return 'User';
  };

  const resolveCreatedAt = (item: any) => {
    return toMillis(item?.createdAt) || toMillis(item?.lastMessageAt) || toMillis(item?.updatedAt) || toMillis(item?.timestamp) || toMillis(item?.sentAt) || null;
  };
    ...
    return rawMessages
      .map((item: any) => {
        ...
        return {
          ...
          text: item?.text || '',
          createdAt,
          type: system ? 'system' : (item?.type === 'file' ? 'file' : 'text'),
    ...
    const loadConversations = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/messenger-conversations?role=teacher&userId=${encodeURIComponent(currentUser)}`,
          { credentials: 'include' }
        );
        const data = await response.json();
        if (!response.ok) throw new Error(data?.error || 'Failed to load conversations.');
        
        const mappedConversations: Conversation[] = (Array.isArray(data) ? data : []).map((item: any) => ({
          id: item.id,
          classId: item.classId,
          name: formatConversationName(item),
          last: item.lastMessage || 'Conversation created.',
          avatar: DEFAULT_AVATAR,
          time: formatConversationTime(item.lastMessageAt || item.updatedAt || item.createdAt),
          ...
          members: Array.isArray(item.participants) ? item.participants.map((p: any) => p?.name).filter(Boolean) : [],
        ...
        mappedConversations.forEach((conversation) => {
          seededMessages[conversation.id] = [{
            ...
            createdAt: toMillis(conversation.createdAt) || toMillis(conversation.updatedAt) || toMillis(conversation.lastMessageAt) || null,
            type: 'system',
          }];
        });
          ...
          setConversations((prev) =>
            prev.map((conversation) =>
              conversation.id === selected.id
                ? {
                    ...conversation,
                    last: lastMessage.type === 'system' ? lastMessage.text : lastMessage.type === 'file' ? `Sent a file: ${lastMessage.fileName}` : lastMessage.text || 'Sent a message',
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((c) => {
      ...
      const memberMatch = c.members?.some((member) => member?.toLowerCase().includes(lowerQuery));
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => flatListRef.current?.scrollToEnd({ animated: false }), 80);
  ...
  const selectedConversationMembers = selected?.members && selected.members.length > 0 ? selected.members : [];
  ...
  const formatConversationName = (item: any) => {
    ...
    if (item.semester && item.schoolYear) return `${subjectWithSection} - ${item.semester} (${item.schoolYear})`;
  ...
  const handleBackFromConversation = () => {
    ...
    setSelected(null);
    setMessageText('');
    setPendingFile(null); // ✅ Clear pending file on back
    onConversationActiveChange?.(false);
  };
  ...
  const refreshFileUrl = async (storagePath: string, conversationId: string): Promise<string | null> => {
    try {
      const response = await fetch(`${API_BASE_URL}/messenger-file-url`, {
        credentials: 'include',
        method: 'POST',
  ...
  // ✅ HELPER: Read file as base64 (Cross-platform Web & Mobile support)
  ...
  // ✅ UPDATED: Pick file and store in pending state (No immediate upload)
  const handlePickFile = async () => {
    if (!selected) return;
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: ['*/*'], // Allow all types, we will block videos manually
      ...
      const realType = await getRealMimeType(asset.uri, asset.mimeType || 'application/octet-stream');
      
      // Block video files again just in case MIME type was detected from extension
      if (realType.startsWith('video/')) {
        alert('Video files are not allowed.');
        return;
      }

      // Read file as base64 (Web compatible)
  ...
  // ✅ UPDATED: Handle sending both File and Text
  const handleSend = async () => {
    ...
    if (currentPendingFile) {
      try {
        ...
        const optimisticFileMsg: Message = {
          ...
          fileUrl: currentPendingFile.uri, // temporary local uri
          fileType: currentPendingFile.mimeType,
          storagePath: '',
        };
        
        setMessagesByConversation((prev) => ({ ...prev, [selected.id]: [...(prev[selected.id] || []), optimisticFileMsg] }));
        setConversations((prev) => prev.map((c) => c.id === selected.id ? { ...c, last: optimisticFileMsg.text, time: formatConversationTime(createdAt), lastMessageAt: createdAt } : c));

        const response = await fetch(`${API_BASE_URL}/messenger-upload-file`, {
          credentials: 'include',
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        // Update optimistic message with real URL and ID
        setMessagesByConversation((prev) => ({
          ...prev,
          [selected.id]: prev[selected.id].map(msg => 
            msg.id === optimisticFileMsg.id 
              ? { ...msg, id: data.data.id, fileUrl: data.data.fileUrl, storagePath: data.data.storagePath } 
              : msg
          ),
        }));
      } catch (error) {
    ...
    if (trimmed) {
      ...
      setMessagesByConversation((prev) => ({ ...prev, [selected.id]: [...(prev[selected.id] || []), optimisticTextMsg] }));
      setConversations((prev) => prev.map((c) => c.id === selected.id ? { ...c, last: trimmed, time: formatConversationTime(textCreatedAt), lastMessageAt: textCreatedAt } : c));
      setSelected((prev) => prev ? { ...prev, last: trimmed, time: formatConversationTime(textCreatedAt), lastMessageAt: textCreatedAt } : prev);
  ...
  // ✅ FIXED: Always refresh URL if storagePath exists
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      if (Platform.OS === 'web') {
        ...
        console.log("Downloading via proxy:", downloadUrl); // Debug log
  ...
  const handleFileDownload = async (item: Message) => {
    ...
    try {
      if (Platform.OS === 'web') {
        ...
        // In handleDownloadImage and handleFileDownload (Web section)
  ...
  const renderImagePreviewModal = () => {
    ...
    return (
      <Modal transparent visible={!!imagePreviewUrl} animationType="fade" onRequestClose={closeImagePreview}>
        <View style={styles.imagePreviewOverlay}>
          <View style={styles.imagePreviewHeader}>
            <Text style={styles.imagePreviewTitle} numberOfLines={1}>{imagePreviewName}</Text>
            <View style={styles.imagePreviewActions}>
              <TouchableOpacity onPress={handleDownloadImage} style={styles.imagePreviewIconButton}>
                <MaterialCommunityIcons name="download" size={24} color="#fff" />
              </TouchableOpacity>
              <TouchableOpacity onPress={closeImagePreview} style={styles.imagePreviewIconButton}>
  ...
  // ✅ NEW: Render Pending File Preview (Facebook Messenger Style)
    ...
    return (
      <View style={styles.pendingFileContainer}>
        <View style={styles.pendingFileInfo}>
          {isImage ? (
            <Image source={{ uri: pendingFile.uri }} style={styles.pendingImagePreview} resizeMode="cover" />
          ) : (
            <View style={styles.pendingFileIcon}>
              <MaterialCommunityIcons name={getFileIcon(pendingFile.mimeType)} size={32} color="#d32f2f" />
            </View>
          )}
          <View style={styles.pendingFileDetails}>
            <Text style={styles.pendingFileName} numberOfLines={1}>{pendingFile.name}</Text>
            <Text style={styles.pendingFileSize}>{(pendingFile.base64.length * 0.75 / 1024).toFixed(1)} KB</Text>
          </View>
          <TouchableOpacity onPress={handleRemovePendingFile} style={styles.removeFileButton}>
  ...
  const FINAL_GRADE_STATUSES: FinalGradeStatus[] = ['passed', 'failed', 'incomplete', 'dropped'];
  ...
  const updateFinalGradeRow = (memberDocId: string, field: 'currentStatus' | 'grade', value: string) => {
    setFinalGradeRows((prev) => prev.map((row) => row.memberDocId === memberDocId ? { ...row, [field]: value } : row));
  };
  ...
  const openAnchoredMenu = () => {
    if (infoButtonRef.current) {
      infoButtonRef.current.measureInWindow((x, y, measuredWidth, measuredHeight) => {
        ...
        setAnchor({ x: x + measuredWidth - menuWidth, y: y + measuredHeight + 6 });
  ...
  const handleOpenFinalGradeModal = async () => {
    ...
    try {
      ...
      if (!membersResponse.ok) throw new Error(membersData?.error || 'Failed to load class members.');
      if (!gradesResponse.ok) throw new Error(gradesData?.error || 'Failed to load final grades.');
      
      const studentMembers = (Array.isArray(membersData) ? membersData : []).filter((member: ClassMember) => member?.role === 'student');
      const savedGradesMap = new Map((Array.isArray(gradesData) ? gradesData : []).map((item: any) => [String(item.studentId || ''), item]));
      
      setClassMembers(studentMembers);
      setFinalGradeRows(studentMembers.map((member: ClassMember) => {
      ...
      if (Platform.OS === 'web') window.alert(error?.message || 'Failed to load class members.');
    } finally {
  ...
  const handleSubmitFinalGrades = async () => {
    ...
    const invalidRow = finalGradeRows.find((row) => !row.currentStatus.trim() || !row.grade.trim());
    if (invalidRow) {
      if (Platform.OS === 'web') window.alert(`Please complete status and grade for ${invalidRow.studentName}.`);
      return;
    }
    ...
    try {
      ...
      let data: any = {};
      try { data = raw ? JSON.parse(raw) : {}; } catch { throw new Error(raw || 'Server returned a non-JSON response.'); }
      if (!response.ok) throw new Error(data?.error || 'Failed to submit final grades.');
      
      setShowFinalGradeModal(false);
      const successMessage = `Final grades submitted for ${finalGradeRows.length} student${finalGradeRows.length > 1 ? 's' : ''}.`;
      const createdAt = Date.now();
      ...
      setMessagesByConversation((prev) => ({ ...prev, [selected.id]: [...(prev[selected.id] || []), optimisticMessage] }));
      setConversations((prev) => prev.map((conversation) => conversation.id === selected.id ? { ...conversation, last: successMessage, time: formatConversationTime(createdAt), lastMessageAt: createdAt } : conversation));
    } catch (error: any) {
      console.error('Submit final grades error:', error);
      if (Platform.OS === 'web') window.alert(error?.message || 'Failed to submit final grades.');
    } finally {
  ...
  const showTimeTooltip = (message: Message) => {
    ...
    target.measureInWindow((x, y, measuredWidth) => {
      ...
      const left = Math.max(8, Math.min(x + measuredWidth / 2 - tooltipWidth / 2, width - tooltipWidth - 8));
  ...
  const renderConversationList = () => (
    <View style={[styles.sidebar, isMobile && styles.sidebarMobile, isSplitView && { width: sizes.sidebarWidth }]}>
      {isMobile && (
        <TouchableOpacity onPress={onBack} style={styles.screenBackButton} activeOpacity={0.8}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              style={[styles.convCard, { paddingHorizontal: sizes.horizontalPadding, paddingVertical: sizes.listRowVertical }, active && styles.convCardActive]}
              onPress={() => handleSelectConversation(item)}
              activeOpacity={0.88}
            >
              <Image source={item.avatar} style={{ width: sizes.listAvatar, height: sizes.listAvatar, borderRadius: sizes.listAvatar / 2, marginRight: isTinyPhone ? 8 : 12 }} />
              <View style={styles.convContent}>
                <View style={styles.convTopRow}>
                  <Text style={[styles.convName, { fontSize: sizes.listName }]} numberOfLines={1}>{item.name}</Text>
                  <Text style={[styles.convTime, { fontSize: sizes.listTime, marginLeft: 8 }]} numberOfLines={1}>{item.time}</Text>
                </View>
                <Text style={[styles.convLast, { fontSize: sizes.listLast }]} numberOfLines={1}>{item.last}</Text>
              </View>
            </TouchableOpacity>
          );
        }}
  ...
  const renderChatHeader = () => (
    <View style={[styles.chatHeader, { height: sizes.headerHeight }]}>
      {isMobile ? (
        <TouchableOpacity onPress={handleBackFromConversation} style={styles.backButton} activeOpacity={0.8}>
          <MaterialCommunityIcons name="chevron-left" size={sizes.headerIcon} color="#333" />
          <Text style={[styles.backText, { fontSize: sizes.backText }]}>Back</Text>
        </TouchableOpacity>
      ) : (<View style={styles.backButton} />)}
      <Text style={[styles.chatTitle, { fontSize: sizes.headerTitle }]} numberOfLines={1}>{selected?.name || 'Select a conversation'}</Text>
      <View style={styles.plusButtonArea}>
        <View ref={infoButtonRef} collapsable={false}>
          <Pressable
            style={[styles.headerIconButton, Platform.OS === 'web' && showInfoMenu && styles.headerIconButtonHover]}
            onPress={handleOpenInfoMenu}
          >
            <MaterialCommunityIcons name="information-outline" size={sizes.headerIcon} color="#111" />
          </Pressable>
        </View>
      </View>
    </View>
    ...
    if (item.type === 'file') {
      ...
      return (
        <View
          style={[
            styles.messageRow,
            { marginBottom: sizes.messageGap, maxWidth: sizes.messageMaxWidth },
            item.fromMe ? styles.messageRowMe : styles.messageRowThem,
          ]}
        >
          {!item.fromMe ? (
            <Text style={[styles.senderName, { fontSize: sizes.senderName }, styles.senderNameThem]}>
              {item.sender}
            </Text>
          ) : null}
          ...
          >
            <View
              style={[
                ...
                { padding: 8, borderRadius: isTinyPhone ? 10 : 16, overflow: 'hidden' },
              ]}
            >
              {isImage ? (
                <Pressable 
                ...
                <Pressable onPress={() => handleFileDownload(item)} style={styles.fileBubbleContent}>
                  <MaterialCommunityIcons name={getFileIcon(item.fileType)} size={32} color={item.fromMe ? '#fff' : '#d32f2f'} />
                  <View style={styles.fileTextContainer}>
                    <Text style={[styles.fileNameText, { color: item.fromMe ? '#fff' : '#111', fontSize: sizes.bubbleText }]} numberOfLines={2}>
                      {item.fileName}
                    </Text>
                    <Text style={[styles.fileSizeText, { color: item.fromMe ? 'rgba(255,255,255,0.8)' : '#666' }]}>
                      Tap to download
                    </Text>
                  </View>
                </Pressable>
        ...
        >
          <View
            style={[
              styles.bubble,
              item.fromMe ? styles.bubbleMe : styles.bubbleThem,
              { paddingHorizontal: isTinyPhone ? 10 : 12, paddingVertical: isTinyPhone ? 8 : 10, borderRadius: isTinyPhone ? 10 : 16 },
            ]}
          >
            <Text style={[styles.bubbleText, { fontSize: sizes.bubbleText }, item.fromMe && styles.bubbleTextMe]}>
              {item.text}
            </Text>
          </View>
  ...
  const renderInfoMenu = () => {
    ...
    return (
      <Modal transparent visible={showInfoMenu} animationType="fade" onRequestClose={() => setShowInfoMenu(false)}>
        <Pressable style={[styles.modalOverlay, styles.professionalOverlay]} onPress={() => setShowInfoMenu(false)}>
          <Pressable
            onPress={(e) => e.stopPropagation()}
            style={[
              styles.professionalModalCard,
              isMobile ? [styles.centeredProfessionalModal, { width: modalWidth, maxHeight: modalMaxHeight }] : { position: 'absolute', width: modalWidth, maxHeight: modalMaxHeight, left: safeLeft, top: safeTop },
            ]}
          >
            <View style={styles.professionalModalHeader}>
              <View style={styles.professionalModalHeaderTextWrap}>
                ...
                <Text style={styles.professionalModalSubtitle}>Choose what you want to do in this class conversation.</Text>
              </View>
              <TouchableOpacity style={styles.professionalCloseButton} onPress={() => setShowInfoMenu(false)} activeOpacity={0.8}>
                <MaterialCommunityIcons name="close" size={18} color="#333" />
              </TouchableOpacity>
            </View>
            <View style={styles.professionalModalScrollContent}>
              <TouchableOpacity style={styles.infoActionCard} activeOpacity={0.85} onPress={handleOpenMembersModal}>
              ...
              {!selected?.isCreatedRoom && (
                <TouchableOpacity style={styles.infoActionCard} activeOpacity={0.85} onPress={handleOpenCreateRoomModal}>
                  <MaterialCommunityIcons name="forum-outline" size={18} color="#222" />
                  <Text style={styles.infoActionCardText}>Create Room</Text>
                </TouchableOpacity>
              )}
              <TouchableOpacity style={styles.infoActionCard} activeOpacity={0.85} onPress={handleOpenFinalGradeModal}>
                <MaterialCommunityIcons name="school-outline" size={18} color="#222" />
                <Text style={styles.infoActionCardText}>Submit Final Grades</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Pressable>
  ...
  const renderCreateRoomModal = () => {
    ...
    return (
      <Modal transparent visible={showCreateRoomModal} animationType="fade" onRequestClose={() => setShowCreateRoomModal(false)}>
        <Pressable style={[styles.modalOverlay, styles.professionalOverlay]} onPress={() => setShowCreateRoomModal(false)}>
          <Pressable
            onPress={(e) => e.stopPropagation()}
            style={[
              styles.professionalModalCard,
          ...
          >
            <View style={styles.professionalModalHeader}>
              <View style={styles.professionalModalHeaderTextWrap}>
                ...
                <Text style={styles.professionalModalSubtitle}>Start a focused discussion with selected members.</Text>
              </View>
              <TouchableOpacity style={styles.professionalCloseButton} onPress={() => setShowCreateRoomModal(false)} activeOpacity={0.8}>
                <MaterialCommunityIcons name="close" size={18} color="#333" />
              </TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={true} contentContainerStyle={styles.professionalModalScrollContent}>
              <View style={styles.professionalSection}>
                <Text style={styles.professionalLabel}>Room name</Text>
                <View style={[styles.professionalInputRow, isTinyPhone && styles.professionalInputRowStack]}>
                  <TextInput value={roomName} onChangeText={setRoomName} placeholder="Enter room name" placeholderTextColor="#9a9a9a" style={[styles.professionalInput, isTinyPhone && styles.professionalInputStack]} />
                  <TouchableOpacity style={[styles.professionalPrimaryButton, isTinyPhone && styles.professionalPrimaryButtonStack]} activeOpacity={0.9} onPress={handleCreateRoom}>
              ...
              <View style={styles.professionalSection}>
                ...
                <Text style={styles.professionalHelperText}>Select who will be included in this discussion room.</Text>
                <View style={styles.professionalMemberList}>
                  {availableRoomMembers.map((member, index) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={`${member}-${index}`}
                        style={[styles.professionalMemberRow, checked && styles.professionalMemberRowActive, isCurrentUser && styles.professionalMemberRowDisabled]}
                        activeOpacity={0.85}
                        ...
                        <View style={styles.professionalMemberInfo}>
                          ...
                          <View style={styles.professionalMemberTextWrap}>
                            ...
                            <Text style={styles.professionalMemberMeta}>
                              {isCurrentUser ? 'Required member' : checked ? 'Selected' : 'Tap to select'}
                            </Text>
                          </View>
                        </View>
                        <MaterialCommunityIcons name={checked ? 'checkbox-marked-circle' : 'checkbox-blank-circle-outline'} size={22} color={checked ? '#d32f2f' : '#b8b8b8'} />
                      </TouchableOpacity>
                    );
                  })}
                </View>
  ...
  const renderMembersModal = () => {
    ...
    return (
      <Modal transparent visible={showMembersModal} animationType="fade" onRequestClose={() => setShowMembersModal(false)}>
        <Pressable style={[styles.modalOverlay, styles.professionalOverlay]} onPress={() => setShowMembersModal(false)}>
          <Pressable
            onPress={(e) => e.stopPropagation()}
            style={[
              styles.professionalModalCard,
          ...
          >
            <View style={styles.professionalModalHeader}>
              <View style={styles.professionalModalHeaderTextWrap}>
                ...
                <Text style={styles.professionalModalSubtitle}>People included in this conversation.</Text>
              </View>
              <TouchableOpacity style={styles.professionalCloseButton} onPress={() => setShowMembersModal(false)} activeOpacity={0.8}>
                <MaterialCommunityIcons name="close" size={18} color="#333" />
              </TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={true} contentContainerStyle={styles.professionalModalScrollContent}>
              {selected?.admin && (
                <View style={styles.conversationMetaCard}>
                  ...
                  {selected.section ? (<Text style={styles.conversationMetaText}>Section: {selected.section}</Text>) : null}
                </View>
              )}
              <View style={styles.professionalMemberList}>
  ...
  const renderFinalGradeModal = () => {
    ...
    return (
      <Modal transparent visible={showFinalGradeModal} animationType="fade" onRequestClose={() => setShowFinalGradeModal(false)}>
        <Pressable style={[styles.modalOverlay, styles.professionalOverlay]} onPress={() => setShowFinalGradeModal(false)}>
          <Pressable
            onPress={(e) => e.stopPropagation()}
            style={[
              styles.professionalModalCard,
          ...
          >
            <View style={styles.professionalModalHeader}>
              <View style={styles.professionalModalHeaderTextWrap}>
                ...
                <Text style={styles.professionalModalSubtitle}>Enter the final grade and status of each class member.</Text>
              </View>
              <TouchableOpacity style={styles.professionalCloseButton} onPress={() => setShowFinalGradeModal(false)} activeOpacity={0.8}>
                <MaterialCommunityIcons name="close" size={18} color="#333" />
              </TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={true} contentContainerStyle={styles.professionalModalScrollContent}>
              {loadingFinalGrades ? (
                ...
                <Text style={styles.professionalHelperText}>{classMembers.length === 0 ? 'No student members found in this class.' : 'No grade rows available.'}</Text>
              ) : (
                <View style={styles.finalGradeList}>
                  {finalGradeRows.map((row) => (
                    <View key={row.memberDocId} style={styles.finalGradeCard}>
                        ...
                        {FINAL_GRADE_STATUSES.map((status) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              activeOpacity={0.85}
                              onPress={() => updateFinalGradeRow(row.memberDocId, 'currentStatus', status)}
                            >
                              <Text style={[styles.statusChipText, active && styles.statusChipTextActive]}>{formatStatusLabel(status)}</Text>
                            </TouchableOpacity>
                          );
                        })}
                      </View>
                      ...
                      <TextInput
                        value={row.grade}
                        onChangeText={(value) => updateFinalGradeRow(row.memberDocId, 'grade', value)}
              ...
              <TouchableOpacity
                style={[styles.professionalPrimaryButton, styles.finalGradeSubmitButton, submittingFinalGrades && { opacity: 0.7 }]}
                activeOpacity={0.9}
              ...
              >
                ...
                <Text style={styles.professionalPrimaryButtonText}>{submittingFinalGrades ? 'Submitting...' : 'Submit Final Grade'}</Text>
              </TouchableOpacity>
            </ScrollView>
          </Pressable>
        </Pressable>
    ...
    if (!selected) {
      return (
        <View style={styles.emptyPane}>
          <MaterialCommunityIcons name="message-text-outline" size={52} color="#bbb" />
          <Text style={styles.emptyPaneTitle}>Select a conversation</Text>
          <Text style={styles.emptyPaneSubtitle}>Choose a message thread from the left to start chatting.</Text>
        </View>
      );
    }
      ...
      <View style={[styles.chatPane, isMobile && styles.chatPaneMobile]}>
        ...
        <FlatList
          ...
          contentContainerStyle={{
            ...
            paddingTop: isMobile ? 90 : 18 * scale,
            paddingBottom: isMobile ? 20 : 24 * scale,
            flexGrow: 1,
        ...
        {hoveredMessageId && hoveredMessageTime ? (
          <View pointerEvents="none" style={[styles.timeTooltip, { left: tooltipPosition.x, top: tooltipPosition.y }]}>
            <Text style={styles.timeTooltipText}>{hoveredMessageTime}</Text>
          </View>
        ) : null}

        {/* ✅ NEW: Pending File Preview */}
        ...
        <View
          style={[
            styles.inputArea,
            {
              ...
              paddingBottom: isMobile ? 10 : isDesktop ? 18 : 12,
              marginBottom: keyboardHeight, // Pushes input up by exact keyboard height
              gap: sizes.inputGap,
            },
          ]}
        >
          ...
          <TouchableOpacity
            style={[
              styles.sendBtn,
              {
                ...
                borderRadius: 18,
                // ✅ UPDATED: Allow sending if there is a pending file OR text
          ...
          >
            {isTinyPhone ? (
              ...
              <Text style={[styles.sendBtnText, { fontSize: sizes.inputText }]}>Send</Text>
            )}
          </TouchableOpacity>
        </View>
    ...
    if (selected) {
      return (
        <SafeAreaView style={styles.mobileScreen}>
          <View style={styles.mobileChatContainer}>{renderChatPane()}</View>
        </SafeAreaView>
      );
    }
    return (
...
const styles = StyleSheet.create({
  ...
  mobileChatContainer: { flex: 1, paddingTop: 50, paddingBottom: 0 },
  desktopScreen: { flex: 1, backgroundColor: '#eef1f5' },
  splitLayout: { flex: 1, flexDirection: 'row', backgroundColor: '#eef1f5', padding: 12, gap: 12 },
  sidebar: { flex: 1, minHeight: 0, backgroundColor: '#fff', flexShrink: 0, borderRadius: 22, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 18, shadowOffset: { width: 0, height: 6 }, elevation: 5 },
  ...
  screenBackButton: { marginTop: 20, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingTop: 14, paddingBottom: 4 },
  screenBackText: { fontSize: 14, fontWeight: '600', color: '#111', marginLeft: 2 },
  pageTitle: { fontWeight: 'bold', paddingBottom: 10, textAlign: 'left', marginTop: 20, paddingHorizontal: 18, color: '#111' },
  convCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', marginBottom: 8, borderRadius: 18, borderWidth: 1, borderColor: '#f0f0f0', shadowColor: '#000', shadowOpacity: 0.03, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 1 },
  ...
  chatPane: { flex: 1, minHeight: 0, backgroundColor: '#fff', minWidth: 0, borderRadius: 22, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 18, shadowOffset: { width: 0, height: 6 }, elevation: 5, overflow: 'hidden' },
  messagesList: { flex: 1, minHeight: 0 },
  chatPaneMobile: { borderRadius: 0, shadowOpacity: 0, elevation: 0 },
  chatHeader: { borderBottomWidth: 1, borderBottomColor: '#ececec', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 14, backgroundColor: '#fff', overflow: 'visible', zIndex: 20 },
  backButton: { minWidth: 60, flexDirection: 'row', alignItems: 'center' },
  backText: { color: '#333' },
  chatTitle: { fontWeight: '700', color: '#111', flex: 1, textAlign: 'center', marginHorizontal: 8 },
  plusButtonArea: { minWidth: 60, alignItems: 'flex-end', justifyContent: 'center', position: 'relative', overflow: 'visible' },
  ...
  input: { alignContent: 'center', justifyContent: 'center', flex: 1, borderWidth: 1, borderColor: '#e1e4e8', color: '#111', backgroundColor: '#f7f8fa' },
  sendBtn: { backgroundColor: '#ea1111', alignItems: 'center', justifyContent: 'center', alignSelf: 'center', flexShrink: 0 },
  sendBtnText: { color: '#fff', fontWeight: '700' },
  emptyPane: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', paddingHorizontal: 24 },
  emptyPaneTitle: { marginTop: 14, fontSize: 22, fontWeight: '700', color: '#222' },
  emptyPaneSubtitle: { marginTop: 6, fontSize: 14, color: '#777', textAlign: 'center', maxWidth: 320 },
  modalOverlay: { flex: 1, backgroundColor: 'transparent' },
  professionalOverlay: { backgroundColor: 'rgba(15, 23, 42, 0.28)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 12 },
  professionalModalCard: { backgroundColor: '#ffffff', borderRadius: 18, borderWidth: 1, borderColor: '#ececec', shadowColor: '#000', shadowOpacity: 0.12, shadowRadius: 14, shadowOffset: { width: 0, height: 6 }, elevation: 8, overflow: 'hidden' },
  centeredProfessionalModal: { position: 'relative' },
  professionalModalHeader: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', paddingHorizontal: 18, paddingTop: 18, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', backgroundColor: '#fff' },
  professionalModalHeaderTextWrap: { flex: 1, paddingRight: 12 },
  professionalModalTitle: { fontSize: 18, fontWeight: '700', color: '#111827' },
  professionalModalSubtitle: { marginTop: 4, fontSize: 12, lineHeight: 18, color: '#6b7280' },
  professionalCloseButton: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: '#f5f5f5' },
  professionalModalScrollContent: { paddingHorizontal: 18, paddingTop: 16, paddingBottom: 18 },
  professionalSection: { marginBottom: 18 },
  professionalLabel: { fontSize: 13, fontWeight: '700', color: '#222', marginBottom: 8 },
  professionalHelperText: { fontSize: 12, lineHeight: 18, color: '#777', marginBottom: 12 },
  professionalInputRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  professionalInputRowStack: { flexDirection: 'column', alignItems: 'stretch' },
  professionalInput: { flex: 1, minHeight: 44, borderWidth: 1, borderColor: '#d6d6d6', backgroundColor: '#fafafa', borderRadius: 12, paddingHorizontal: 14, fontSize: 14, color: '#111' },
  professionalInputStack: { width: '100%' },
  professionalPrimaryButton: { height: 44, minWidth: 108, paddingHorizontal: 16, borderRadius: 12, backgroundColor: '#d32f2f', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  professionalPrimaryButtonStack: { width: '100%' },
  professionalPrimaryButtonText: { color: '#fff', fontSize: 13, fontWeight: '700' },
  professionalMemberList: { gap: 10 },
  professionalMemberRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', minHeight: 58, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderColor: '#ececec', backgroundColor: '#fff' },
  professionalMemberRowActive: { borderColor: '#f2b2b2', backgroundColor: '#fff7f7' },
  professionalMemberRowDisabled: { opacity: 0.92 },
  professionalMemberRowStatic: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', minHeight: 56, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderColor: '#ececec', backgroundColor: '#fbfbfb' },
  professionalMemberInfo: { flexDirection: 'row', alignItems: 'center', flex: 1, minWidth: 0, paddingRight: 12 },
  professionalMemberAvatar: { width: 34, height: 34, borderRadius: 17, backgroundColor: '#f2f2f2', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  professionalMemberTextWrap: { flex: 1, minWidth: 0 },
  professionalMemberName: { fontSize: 13, fontWeight: '600', color: '#222' },
  professionalMemberMeta: { marginTop: 2, fontSize: 11, color: '#7a7a7a' },
  infoActionCard: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 14, paddingHorizontal: 14, borderRadius: 14, borderWidth: 1, borderColor: '#ededed', backgroundColor: '#fafafa', marginBottom: 10 },
  infoActionCardText: { fontSize: 14, fontWeight: '600', color: '#222' },
  finalGradeList: { gap: 14 },
  finalGradeCard: { borderWidth: 1, borderColor: '#ececec', borderRadius: 14, backgroundColor: '#fcfcfc', padding: 14, marginBottom: 12 },
  finalGradeTop: { marginBottom: 12 },
  statusChipsWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 },
  statusChip: { paddingHorizontal: 12, paddingVertical: 8, borderRadius: 999, borderWidth: 1, borderColor: '#d8d8d8', backgroundColor: '#fff' },
  statusChipActive: { backgroundColor: '#d94c43', borderColor: '#d94c43' },
  statusChipText: { fontSize: 12, fontWeight: '600', color: '#444' },
  statusChipTextActive: { color: '#fff' },
  finalGradeSubmitButton: { marginTop: 8, alignSelf: 'stretch', justifyContent: 'center' },
  conversationMetaCard: { marginBottom: 12, padding: 12, borderRadius: 12, backgroundColor: '#fff7f7', borderWidth: 1, borderColor: '#f5d0d0' },
  conversationMetaTitle: { fontSize: 12, fontWeight: '700', color: '#222', marginBottom: 6 },
  conversationMetaText: { fontSize: 12, color: '#555', marginTop: 2 },

  // File Message Styles
  attachButton: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center', borderRadius: 21, backgroundColor: '#f0f2f5' },
  fileBubbleContent: { flexDirection: 'row', alignItems: 'center', padding: 10, minWidth: 200, gap: 12 },
  fileTextContainer: { flex: 1 },
  fileNameText: { fontWeight: '600' },
  fileSizeText: { fontSize: 11, marginTop: 2 },
  fileImageOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.5)', paddingHorizontal: 8, paddingVertical: 6, flexDirection: 'row', alignItems: 'center', gap: 6, borderBottomLeftRadius: 12, borderBottomRightRadius: 12 },
  fileImageName: { color: '#fff', fontSize: 12, fontWeight: '600', flex: 1 },
  
  // Image Preview Styles
  imagePreviewOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.9)', justifyContent: 'center', alignItems: 'center' },
  imagePreviewHeader: { position: 'absolute', top: 50, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, zIndex: 10 },
  imagePreviewTitle: { color: '#fff', fontSize: 16, fontWeight: '600', flex: 1, marginRight: 10 },
  imagePreviewActions: { flexDirection: 'row', gap: 15 },
  imagePreviewIconButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  imagePreviewContent: { flex: 1, width: '100%', justifyContent: 'center', alignItems: 'center' },
  imagePreviewImage: { width: '100%', height: '100%' },

  // ✅ NEW: Pending File Preview Styles
