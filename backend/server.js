...
app.post("/auth/send-forgot-password-pin", async (req, res) => {
  try {
    ...
    // 2. Find User Across All Role Collections
    const foundUser = await findUserByEmailAcrossRoles(trimmedEmail);
    
    if (!foundUser) {
      // Security Note: In production, you might want to return success 
      // even if user not found to prevent email enumeration.
      // For now, keeping your existing behavior:
      return res.status(404).json({ error: "No account found for this email." });
    }

    const userData = foundUser.data || {};
    const userId = userData.studentId || userData.teacherId || userData.adminId || foundUser.id;

    // 3. Generate PIN & Expiry
    const forgotPasswordPin = generateForgotPasswordPin(4);
    const forgotPasswordPinExpiresAt = buildPinExpiryDate(15); // 15 minutes

    // 4. Update Firestore FIRST (so PIN exists before email arrives)
    ...
    // 5. SEND EMAIL SYNCHRONOUSLY (Critical Fix)
    // We await this so any Resend errors are caught in this try/catch
    ...
    // 6. Only respond success AFTER email is confirmed sent
    return res.json({
      success: true,
      role: foundUser.role,
      id: userId,
    ...
    // Return specific Resend errors to help debug
    const errorMessage = error?.message || "Failed to send forgot password PIN.";
